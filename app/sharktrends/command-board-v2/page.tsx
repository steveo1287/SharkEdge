...
export default async function CommandBoardV2Page({ searchParams }: PageProps) {
  ...
  return (
    <main className="mx-auto grid max-w-7xl gap-5 px-4 py-6 sm:px-6 lg:px-8">
      ...
      {hasDataGap ? <section className="rounded-[1.5rem] border border-amber-300/20 bg-amber-300/10 p-4 text-sm leading-6 text-amber-100"><div className="font-semibold text-white">Data pipe still incomplete</div><div className="mt-2">Generated attachments or market context are still missing from this view. The board is showing fallback native/raw games until the provider and generated-system attachment layers start producing usable rows.</div><div className="mt-3 flex flex-wrap gap-2"><Link href="/sharktrends/ingestion-health" className="rounded-xl border border-amber-300/25 bg-black/20 px-3 py-2 text-[10px] font-semibold uppercase tracking-[0.14em] text-amber-100">Ingestion health</Link><Link href="/sharktrends/market-data-source" className="rounded-xl border border-amber-300/25 bg-black/20 px-3 py-2 text-[10px] font-semibold uppercase tracking-[0.14em] text-amber-100">Market source</Link><a href="/api/admin/ingest/odds-api-io?sport=baseball&league=MLB&status=upcoming&eventLimit=5&dryRun=true" className="rounded-xl border border-amber-300/25 bg-black/20 px-3 py-2 text-[10px] font-semibold uppercase tracking-[0.14em] text-amber-100">Run dry test</a><Link href="/sharktrends?mode=all" className="rounded-xl border border-amber-300/25 bg-black/20 px-3 py-2 text-[10px] font-semibold uppercase tracking-[0.14em] text-amber-100">Debug all games</Link></div></section> : null}
      {usingFallback ? <section className="rounded-[1.5rem] border border-cyan-300/20 bg-cyan-300/10 p-4 text-sm leading-6 text-cyan-100"><div className="font-semibold text-white">Fallback view active</div><div className="mt-2">No command-ready games cleared the stack, so SharkTrends is showing the best available games instead of an empty page. Fix the data pipe to replace this fallback with generated and market-backed command cards.</div></section> : null}
      <section className="grid gap-3 md:grid-cols-2 xl:grid-cols-5"><Metric label="Shown games" value={visibleGames.length} note={usingFallback ? "Fallback games shown." : "Command games shown."} /><Metric label="Hidden raw" value={incompleteGames.length} note="Incomplete games collapsed below." /><Metric label="Generated attached" value={payload.stats.generatedAttached ?? 0} note="Generated systems attached to games." /><Metric label="Verified generated" value={payload.stats.verifiedGenerated} note="Fully verified generated systems." /><Metric label="Market sourced" value={payload.stats.marketSourced} note="Games with market context." /></section>
      ...
      <section className="grid gap-4">{visibleGames.length ? visibleGames.map((game) => <GameCard key={game.eventId} game={game} />) : <div className="rounded-[1.5rem] border border-white/10 bg-slate-950/60 p-4 text-sm leading-6 text-slate-400">No games are available under this view. Open debug mode and ingestion health to inspect the data path.</div>}</section>
