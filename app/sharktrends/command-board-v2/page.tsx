  ...
  return (
    <article className="rounded-[1.5rem] border border-white/10 bg-slate-950/70 p-4">
      <div className="flex flex-wrap items-start justify-between gap-3">
        ...
        <div className="grid grid-cols-4 gap-2 text-center text-[10px] uppercase tracking-[0.12em] text-slate-500"><div className="rounded-xl border border-white/10 bg-black/25 px-2 py-2"><div className="text-sm font-semibold text-white">{game.counts.native}</div>native</div><div className="rounded-xl border border-white/10 bg-black/25 px-2 py-2"><div className="text-sm font-semibold text-white">{game.counts.generated}</div>gen attach</div><div className="rounded-xl border border-white/10 bg-black/25 px-2 py-2"><div className="text-sm font-semibold text-white">{game.counts.generatedVerified ?? 0}</div>verified</div><div className="rounded-xl border border-white/10 bg-black/25 px-2 py-2"><div className="text-sm font-semibold text-white">{game.counts.blockers}</div>blocks</div></div>
      </div>
...
export default async function CommandBoardV2Page({ searchParams }: PageProps) {
  ...
  return (
    <main className="mx-auto grid max-w-7xl gap-5 px-4 py-6 sm:px-6 lg:px-8">
      ...
      <section className="grid gap-3 md:grid-cols-2 xl:grid-cols-5"><Metric label="Games" value={payload.stats.games} note="Composed matchup cards." /><Metric label="Action" value={payload.stats.action} note="Full signal stack aligned." /><Metric label="Generated attached" value={payload.stats.generatedAttached ?? 0} note="Generated systems attached to games, including pending verification." /><Metric label="Verified generated" value={payload.stats.verifiedGenerated} note="Fully verified generated systems attached." /><Metric label="Market sourced" value={payload.stats.marketSourced} note="Games with sourced market context." /></section>
      ...
      <section className="grid gap-4">{payload.games.length ? payload.games.map((game) => <GameCard key={game.eventId} game={game} />) : <div className="rounded-[1.5rem] border border-white/10 bg-slate-950/60 p-4 text-sm leading-6 text-slate-400">No command-board games are available under this view. Check ingestion health and market source coverage.</div>}</section>
    </main>
  );
}
