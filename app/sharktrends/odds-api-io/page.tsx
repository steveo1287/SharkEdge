  ...
  return (
    <main className="mx-auto grid max-w-7xl gap-5 px-4 py-6 sm:px-6 lg:px-8">
      <section className="rounded-[1.75rem] border border-cyan-300/15 bg-slate-950/70 p-5 shadow-[0_0_60px_rgba(14,165,233,0.10)]">
        <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
          ...
          <div className="flex flex-wrap gap-2 text-xs font-semibold uppercase tracking-[0.14em]">
            <Link href="/sharktrends/ingestion-health" className="text-cyan-200 hover:text-cyan-100">Ingestion health</Link>
      ...
      <section className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
        ...
        <Metric label="Write secret" value={writeProtected ? "set" : "missing"} note="Set CRON_SECRET or ODDS_API_IO_INGEST_SECRET for dryRun=false." />
      ...
      <section className="rounded-[1.5rem] border border-cyan-300/15 bg-cyan-300/[0.035] p-4">
        ...
        <div className="grid gap-3 md:grid-cols-4">
          ...
          <Link href="/sharktrends/ingestion-health" className="rounded-xl border border-white/10 bg-black/25 px-4 py-3 text-xs font-semibold uppercase tracking-[0.14em] text-slate-300 hover:border-cyan-300/25">View run log</Link>
        ...
        <div className="mt-3 text-xs leading-5 text-slate-400">
          Write mode uses the same endpoint with dryRun=false and a valid ingest secret through a Bearer token, x-ingest-secret header, x-cron-secret header, or secret query parameter.
        </div>
      </section>
      ...
      <section className="rounded-[1.5rem] border border-white/10 bg-slate-950/60 p-4">
        ...
        <div className="mt-3 grid gap-2 text-sm leading-6 text-slate-300">
          ...
          <div><span className="font-semibold text-white">CRON_SECRET</span> or <span className="font-semibold text-white">ODDS_API_IO_INGEST_SECRET</span> — write-mode protection for ingestion.</div>
