...
export default async function MlbStatTrendsPage({ searchParams }: PageProps) {
  ...
  return (
    <main className="mx-auto grid max-w-7xl gap-5 px-4 py-6 sm:px-6 lg:px-8">
      <section className="rounded-[1.75rem] border border-cyan-300/15 bg-slate-950/75 p-5 shadow-[0_0_70px_rgba(14,165,233,0.10)]"><div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between"><div><div className="text-xs font-semibold uppercase tracking-[0.24em] text-cyan-300">MLB elite trends</div><h1 className="mt-2 font-display text-3xl font-semibold text-white md:text-4xl">Decision-grade MLB trend board</h1><p className="mt-2 max-w-3xl text-sm leading-6 text-slate-400">Built from official schedule data, recent final scores, probable pitchers, and pitcher stat logs. The elite layer adds score, actionability, and hard gates so stat angles do not masquerade as finished bets.</p></div><div className="flex flex-wrap gap-3 text-xs font-semibold uppercase tracking-[0.14em]"><Link href="/trends?league=MLB" className="text-cyan-200 hover:text-cyan-100">Main trends</Link><Link href="/sharktrends/mlb-spine" className="text-cyan-200 hover:text-cyan-100">MLB spine</Link><Link href="/api/mlb/stat-trends" className="text-cyan-200 hover:text-cyan-100">JSON</Link></div></div></section>
      <section className="grid gap-3 md:grid-cols-2 xl:grid-cols-6"><Metric label="Games" value={payload.stats.games} note={`MLB games on ${payload.date}.`} /><Metric label="Decision avg" value={payload.stats.avgDecisionScore} tone={payload.stats.avgDecisionScore >= 70 ? "good" : payload.stats.avgDecisionScore >= 55 ? "warn" : "neutral"} note="Average score across visible stat cards." /><Metric label="Price required" value={payload.stats.priceRequired} tone={payload.stats.priceRequired ? "good" : "neutral"} note="Cards with usable stat stack awaiting market edge." /><Metric label="Watchlist" value={payload.stats.watchlist} tone={payload.stats.watchlist ? "warn" : "neutral"} note="Context cards below premium placement." /><Metric label="Pass" value={payload.stats.pass} tone={payload.stats.pass ? "bad" : "neutral"} note="Cards rejected by decision gates." /><Metric label="Pitcher TBD" value={payload.stats.missingProbablePitchers} tone={payload.stats.missingProbablePitchers ? "warn" : "good"} note="Probable pitcher slots missing or unconfirmed." /></section>
