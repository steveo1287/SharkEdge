...
import { ACTIVE_UFC_NAMED_FIGHTER_PRIOR_BATCH_1D } from "@/services/ufc/active-named-fighter-prior-batch1d";
...
function findPrior(row: Row, only: string | null) {
  const tokens = [row.fighter_id, row.full_name, row.nickname, asRecord(row.payload_json).slug, asRecord(row.payload_json).fighterSlug]
    ...
    .filter(Boolean);
  return ACTIVE_UFC_NAMED_FIGHTER_PRIOR_BATCH_1D.find((prior) => {
...
function buildPayload(prior: (typeof ACTIVE_UFC_NAMED_FIGHTER_PRIOR_BATCH_1D)[number], signals: string[]) {
  ...
  return {
    ...
    fighterTendencies: {
      ...
      generatedAt,
      source: "active-ufc-named-fighter-era-prior-batch1d",
    ...
    tendencyProfile: { source: "active-ufc-named-fighter-era-prior-batch1d", generatedAt, archetype: String(tendencyPrior.archetype ?? prior.metadata.styleOverride), confidence: 0.94, sourceQuality: "A", fallbackUsed: false, missingSignals: [], namedPriorIds: [prior.id], activeSignals: signals },
...
function featurePayload(prior: (typeof ACTIVE_UFC_NAMED_FIGHTER_PRIOR_BATCH_1D)[number], signals: string[]) {
  return {
    ...prior.profile,
    ...
    eliteCombatCredentialPrior: { source: "active-ufc-named-fighter-era-prior-batch1d", confidence: prior.confidence, sourceUrl: prior.sourceUrl, appliedPriors: [{ id: prior.id, confidence: prior.confidence, changedKeys: Object.keys(prior.profile), activeSignals: signals }], evidence: prior.evidence }
  };
}
...
async function apply(row: Row, prior: (typeof ACTIVE_UFC_NAMED_FIGHTER_PRIOR_BATCH_1D)[number], modelVersion: string, signals: string[]) {
...
export async function GET(request: Request) {
  ...
  return NextResponse.json({ ok: errors.length === 0, batch: "1D", dryRun, activeOnly, scanned: rows.length, matched, applied: applied.length, skippedInactive: skipped.length, priorsApplied: applied, skipped, errors }, { status: errors.length ? 500 : 200 });
}
