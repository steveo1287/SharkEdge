...
function authorized(request: Request) {
  ...
  return url.searchParams.get("confirm") === "build-elite-profiles";
}
...
export async function GET(request: Request) {
  if (!authorized(request)) {
    return NextResponse.json({ ok: false, error: "unauthorized", required: process.env.UFC_ADMIN_RUN_TOKEN ? "valid token" : "?confirm=build-elite-profiles" }, { status: 401 });
  }
  ...
  try {
    const result = await buildEliteUfcFighterProfiles({ dryRun, limit, horizonDays, modelVersion });
