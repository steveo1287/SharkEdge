...
import { persistUfcEnrichmentLanes } from "@/services/ufc/persist-enrichment-lanes";
...
function authorized(request: Request) {
  ...
  return url.searchParams.get("confirm") === "enrichment-lanes";
}
...
export async function GET(request: Request) {
  if (!authorized(request)) {
    return NextResponse.json({ ok: false, error: "unauthorized", required: process.env.UFC_ADMIN_RUN_TOKEN ? "valid token" : "?confirm=enrichment-lanes" }, { status: 401 });
  }

  const url = new URL(request.url);
  const dryRun = boolParam(url, "dryRun", true);
  const horizonDays = numberParam(url, "horizonDays", 180, 1, 365);
  const limit = numberParam(url, "limit", 500, 1, 1000);

  try {
    const result = await persistUfcEnrichmentLanes({ dryRun, horizonDays, limit });
