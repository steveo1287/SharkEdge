...
function PriorityTable({ rows }: { rows: Row[] }) {
  ...
  return (
    <SimTableShell title="Priority queue" description="Only the first decisions worth checking. Open the league workspace for the full board.">
...
export default async function SimHubPage() {
  ...
  const workspaces: WorkspaceConfig[] = [
    { href: "/sim/nba", eyebrow: "NBA workspace", title: "Player Sims + Side Queue", description: "Calibrated player box scores, prop drilldowns, confidence gates, and side reads in one tight board.", primaryMetric: `${nbaRows.length} games`, secondaryMetric: `${nbaPlayers} players`, action: "Open NBA desk" },
    { href: "/sim/mlb", eyebrow: "MLB workspace", title: "Sides + Totals Edge Desk", description: "Moneyline, total edge, pitcher/bullpen factors, market-line matching, and MLB Data API player-model status.", primaryMetric: `${mlbRows.length} games`, secondaryMetric: `${mlbMatchedLines}/${mlbRows.length} lines`, action: "Open MLB desk" },
    { href: "/sim/players?league=NBA", eyebrow: "NBA drilldown", title: "Projected Player Box Scores", description: "Use this when the player prop board needs exact points, boards, assists, threes, PRA, floor and ceiling.", primaryMetric: `${nbaPlayers} rows`, secondaryMetric: "calibrated", action: "Open player board" }
  ];
  ...
  return (
    <div className="space-y-6">
      <SimWorkspaceHeader
        ...
        description="The hub is now a routing layer, not a dump. NBA and MLB have separate boards because the betting questions are different: NBA needs player/prop sims; MLB needs sides, totals, pitching, bullpen, and market confirmation."
      >
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
          <SimMetricTile label="Actionable" value={`${attack}/${watch}`} sub="Attack / watch signals" emphasis="strong" />
          <SimMetricTile label="NBA slate" value={String(nbaRows.length)} sub={`${nbaPlayers} player rows`} />
          <SimMetricTile label="MLB slate" value={String(mlbRows.length)} sub={`${mlbMatchedLines}/${mlbRows.length} matched lines`} />
          <SimMetricTile label="MLB Data" value={`${realMlbData}/${mlbRows.length}`} sub={`ML rows ${mlModel?.rows ?? 0}${calibration?.ok ? ` · ECE ${calibration.ece.toFixed(3)}` : ""}`} />
        </div>
      </SimWorkspaceHeader>
