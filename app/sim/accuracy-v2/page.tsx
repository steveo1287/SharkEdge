...
function selectedWindow(value?: string) { const normalized = String(value ?? "30").toLowerCase(); return WINDOWS.find((item) => item.value === normalized || String(item.days) === normalized) ?? WINDOWS[1]; }
function pct(value: number | null | undefined) { return typeof value === "number" && Number.isFinite(value) ? `${(value * 100).toFixed(1)}%` : "—"; }
function record(wins = 0, losses = 0, pushes = 0) { return pushes ? `${wins}-${losses}-${pushes}` : `${wins}-${losses}`; }
function units(value: number | null | undefined) { return typeof value === "number" && Number.isFinite(value) ? `${value > 0 ? "+" : ""}${value.toFixed(1)}u` : "—"; }
function when(value?: string | null) { if (!value) return "—"; const date = new Date(value); return Number.isNaN(date.getTime()) ? "—" : new Intl.DateTimeFormat("en-US", { month: "short", day: "numeric", hour: "numeric", minute: "2-digit" }).format(date); }
function href(params: Record<string, string | null | undefined>) { const query = new URLSearchParams(); for (const [key, value] of Object.entries(params)) if (value) query.set(key, value); return `/sim/accuracy/mlb?${query.toString()}`; }
function total(summaries: Array<{ predictionCount: number; settledCount: number; pendingCount: number; winCount: number; lossCount: number; pushCount: number }>) {
  const out = summaries.reduce((acc, row) => ({ predictionCount: acc.predictionCount + row.predictionCount, settledCount: acc.settledCount + row.settledCount, pendingCount: acc.pendingCount + row.pendingCount, winCount: acc.winCount + row.winCount, lossCount: acc.lossCount + row.lossCount, pushCount: acc.pushCount + row.pushCount }), { predictionCount: 0, settledCount: 0, pendingCount: 0, winCount: 0, lossCount: 0, pushCount: 0 });
  return { ...out, winRate: out.winCount + out.lossCount > 0 ? out.winCount / (out.winCount + out.lossCount) : null };
}
function Card({ label, value, note }: { label: string; value: string; note: string }) { return <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-4"><div className="text-[10px] font-black uppercase tracking-[0.18em] text-slate-500">{label}</div><div className="mt-2 font-mono text-2xl font-bold text-white">{value}</div><div className="mt-2 text-xs leading-5 text-slate-400">{note}</div></div>; }
...
export default async function MlbAccuracyPage({ searchParams }: PageProps) {
  ...
  return <main className="mx-auto grid max-w-7xl gap-5 px-4 py-6 sm:px-6 lg:px-8">
    <section className="rounded-[1.75rem] border border-cyan-300/15 bg-slate-950/80 p-5"><div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between"><div><div className="text-xs font-black uppercase tracking-[0.24em] text-cyan-300">MLB Accuracy</div><h1 className="mt-2 font-display text-4xl font-black tracking-[-0.05em] text-white">Baseball records only</h1><p className="mt-2 max-w-3xl text-sm leading-6 text-slate-400">This page is isolated from UFC. It only counts MLB moneyline, full-game totals, first-five, and NRFI/YRFI audit rows.</p><div className="mt-3 text-xs text-slate-500">Generated {when(ledger.generatedAt)} · Source: sim_prediction_snapshots</div></div><div className="flex flex-wrap gap-3 text-xs font-bold uppercase tracking-[0.14em]"><Link href="/sim/accuracy" className="text-cyan-200">Hub</Link><Link href="/accuracy/mma" className="text-cyan-200">UFC Accuracy</Link><Link href="/api/sim/accuracy" className="text-cyan-200">API</Link></div></div></section>
