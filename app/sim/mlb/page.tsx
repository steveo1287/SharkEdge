...
function PickCard({ pick }: { pick: MlbSimPick }) {
  return (
    <Link href={`/sim/mlb/${encodeURIComponent(pick.gameId)}`} className="rounded-[1.1rem] border border-white/10 bg-[#06101b]/82 p-4 transition hover:border-aqua/35 hover:bg-aqua/[0.045]">
      ...
      <div className="mt-4 grid grid-cols-3 gap-2"><SimMetricTile label="Sim %" value={pct(pick.modelProbability)} sub={sourceLabel(pick.source)} /><SimMetricTile label="Projected" value={pick.projectedScore} sub={`total ${num(pick.projectedTotal, 1)}`} /><SimMetricTile label="Score" value={String(pick.score)} sub={`conf ${pct(pick.confidence)}`} /></div>
      <div className="mt-3 flex flex-wrap gap-2"><Badge label={`quality ${pick.dataQuality}`} />{pick.projectedRunEdge != null ? <Badge label={`run ${plus(pick.projectedRunEdge)}`} tone="aqua" /> : null}</div>
