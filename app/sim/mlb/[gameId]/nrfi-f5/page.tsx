...
export default async function MlbNrfiF5Page({ params }: PageProps) {
  ...
  if (!game) notFound();
  const view = game.nrfiF5;
  return (
    <div className="space-y-5">
      <SimWorkspaceHeader eyebrow="MLB Game Center" title="NRFI / F5" description={`${game.teams.away.name} @ ${game.teams.home.name} - first inning and first five view`} actions={[{ href: `/sim/mlb/${encodeURIComponent(game.gameId)}`, label: "Summary" }, { href: "/sim/mlb", label: "MLB Board", tone: "primary" }]} />
      <MlbFranchiseTabs gameId={game.gameId} active="nrfi-f5" />
      <section className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
        <FranchiseStat label="NRFI" value={pct(view.nrfiPct)} sub="No run first inning" />
        <FranchiseStat label="YRFI" value={pct(view.yrfiPct)} sub="Run in first inning" />
        <FranchiseStat label="F5 Total" value={num(view.f5Total)} sub={`${game.teams.away.name} ${num(view.f5AwayRuns)} - ${game.teams.home.name} ${num(view.f5HomeRuns)}`} />
        <FranchiseStat label="F5 Over 4.5" value={pct(view.f5Over45Pct)} sub="projection probability" />
      </section>
      <section className="grid gap-4 lg:grid-cols-2">
        <SimSignalCard><Bar left="NRFI" right="YRFI" leftPct={view.nrfiPct} /></SimSignalCard>
        <SimSignalCard>
          <Bar left={game.teams.away.name} right={game.teams.home.name} leftPct={view.f5AwayWinPct} />
      ...
      <FranchiseTable title="Inning Shape" description="Expected runs and no-run probability by inning, 1 through 5.">
