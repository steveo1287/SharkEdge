    ...
    <SimSignalCard className="border-sky-400/30 bg-sky-500/[0.06]">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
        ...
        <div className="grid gap-2 sm:grid-cols-5 lg:min-w-[620px]">
          <SimMetricTile label="Minutes" value={num(player.projectedMinutes)} />
          <SimMetricTile label="Points" value={num(player.projectedPoints)} emphasis="strong" />
          <SimMetricTile label="Reb" value={num(player.projectedRebounds)} />
          <SimMetricTile label="Ast" value={num(player.projectedAssists)} />
          <SimMetricTile label="PRA" value={num(pra)} />
        </div>
      </div>
      <div className="mt-3 grid gap-2 lg:grid-cols-3">
        <div className="rounded-xl border border-white/10 bg-slate-950/40 p-3 text-xs text-slate-300"><div className="mb-1 text-slate-100">Floor / Median / Ceiling</div><div>PTS {num(player.floor.points)} / {num(player.projectedPoints)} / {num(player.ceiling.points)}</div><div>REB {num(player.floor.rebounds)} / {num(player.projectedRebounds)} / {num(player.ceiling.rebounds)}</div><div>AST {num(player.floor.assists)} / {num(player.projectedAssists)} / {num(player.ceiling.assists)}</div></div>
        ...
        <div className="rounded-xl border border-white/10 bg-slate-950/40 p-3 text-xs text-slate-300"><div className="mb-1 text-slate-100">Why / risk</div><div>{player.whyLikely[0] ?? "Role and possession context support the median projection."}</div><div className="mt-1 text-slate-500">{player.whyNotLikely[0] ?? "No major downside flags from current context stack."}</div></div>
      </div>
    </SimSignalCard>
  );
}
...
function PlayerBoxScoreTable({ title, rows, focusPlayer, targetPoints }: { title: string; rows: PlayerProjection[]; focusPlayer: string | null; targetPoints: number }) {
  const totals = { minutes: sum(rows, (row) => row.projectedMinutes), points: sum(rows, (row) => row.projectedPoints), rebounds: sum(rows, (row) => row.projectedRebounds), assists: sum(rows, (row) => row.projectedAssists), threes: sum(rows, (row) => row.projectedThrees) };
  return (
    <SimTableShell title={title} description="Calibrated projected player box score" right={<div className="text-right font-mono text-sm text-sky-200">{num(totals.points)} / {num(targetPoints)} pts</div>}>
      <table className="min-w-full text-left text-xs">
        ...
        <tbody>
          {rows.map((row) => { const pra = row.projectedPoints + row.projectedRebounds + row.projectedAssists; const focused = playerMatches(row, focusPlayer); return (
            <tr key={`${row.teamName}:${row.playerName}`} className={focused ? "border-b border-sky-400/20 bg-sky-500/[0.08]" : "border-b border-white/5 last:border-none"}>
              <td className="px-3 py-2"><div className="flex items-center gap-2"><span className="font-semibold text-white">{row.playerName}</span>{focused ? <Badge tone="brand">FOCUS</Badge> : null}</div><div className="text-[10px] text-slate-500">{row.status} · {row.source}</div></td>
              <td className="px-3 py-2 text-right font-mono text-slate-200">{num(row.projectedMinutes)}</td><td className="px-3 py-2 text-right font-mono text-sky-200">{num(row.projectedPoints)}</td><td className="px-3 py-2 text-right font-mono text-slate-200">{num(row.projectedRebounds)}</td><td className="px-3 py-2 text-right font-mono text-slate-200">{num(row.projectedAssists)}</td><td className="px-3 py-2 text-right font-mono text-slate-200">{num(row.projectedThrees)}</td><td className="px-3 py-2 text-right font-mono text-slate-200">{num(pra)}</td><td className="px-3 py-2 text-right font-mono text-slate-200">{pct(row.confidence, 0)}</td>
            </tr>
          ); })}
          <tr className="bg-white/[0.025] font-semibold text-white"><td className="px-3 py-2">Team total</td><td className="px-3 py-2 text-right font-mono">{num(totals.minutes)}</td><td className="px-3 py-2 text-right font-mono">{num(totals.points)}</td><td className="px-3 py-2 text-right font-mono">{num(totals.rebounds)}</td><td className="px-3 py-2 text-right font-mono">{num(totals.assists)}</td><td className="px-3 py-2 text-right font-mono">{num(totals.threes)}</td><td className="px-3 py-2 text-right font-mono">{num(totals.points + totals.rebounds + totals.assists)}</td><td className="px-3 py-2 text-right font-mono">--</td></tr>
        </tbody>
      </table>
    </SimTableShell>
  );
...
function MatchupBoxScoreCard({ row, focusPlayer, focusedGameId }: { row: MatchupRow; focusPlayer: string | null; focusedGameId: string | null }) {
  ...
  return (
    <section className={focusedGame ? "grid gap-4 rounded-3xl border border-sky-400/30 bg-sky-500/[0.045] p-4" : "grid gap-4 rounded-3xl border border-white/10 bg-white/[0.025] p-4"}>
      <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between"><div><div className="text-[10px] uppercase tracking-[0.2em] text-slate-500">{game.leagueKey} · {formatTime(game.startTime)}</div><h2 className="mt-2 font-display text-2xl font-semibold text-white">{projection.matchup.away} @ {projection.matchup.home}</h2><p className="mt-2 max-w-3xl text-sm leading-6 text-slate-400">{projection.read}</p></div><div className="flex flex-wrap gap-2 lg:justify-end"><SimStatusBadge status={game.status} /><SimDecisionBadge tier={tier} />{focusedGame ? <Badge tone="brand">MATCHED PROP</Badge> : null}<Link href={`/sim/${game.leagueKey.toLowerCase()}/${encodeURIComponent(game.id)}`} className="rounded-full border border-sky-400/35 bg-sky-500/10 px-4 py-2 text-xs font-semibold uppercase tracking-[0.14em] text-sky-200 hover:bg-sky-500/15">Open full sim</Link></div></div>
      {focusedPlayer ? <PlayerFocusPanel player={focusedPlayer} game={game} /> : null}
      <div className="grid gap-3 md:grid-cols-5"><SimMetricTile label="Away score" value={num(projection.distribution.avgAway)} sub={projection.matchup.away} /><SimMetricTile label="Home score" value={num(projection.distribution.avgHome)} sub={projection.matchup.home} /><SimMetricTile label="Box pts" value={`${num(awayPoints)} / ${num(homePoints)}`} sub="Calibrated player sum" emphasis="strong" /><SimMetricTile label="Game total" value={num(projection.nbaIntel?.projectedTotal ?? awayPoints + homePoints)} sub="Projection engine" /><SimMetricTile label="Confidence" value={pct(projection.nbaIntel?.confidence, 0)} sub={`${projection.nbaIntel?.playerStatProjections.length ?? 0} player rows`} /></div>
      <PlayerVsPlayerPanel awayRows={awayRows} homeRows={homeRows} focusPlayer={focusPlayer} />
      {players.length ? <div className="grid gap-4 xl:grid-cols-2"><PlayerBoxScoreTable title={projection.matchup.away} rows={awayRows} focusPlayer={focusPlayer} targetPoints={projection.distribution.avgAway} /><PlayerBoxScoreTable title={projection.matchup.home} rows={homeRows} focusPlayer={focusPlayer} targetPoints={projection.distribution.avgHome} /></div> : <SimSignalCard className="text-sm leading-6 text-slate-400">Player projection rows were not returned for this matchup. The game-level sim is available, but the roster box score needs NBA player profiles from the projection service.</SimSignalCard>}
    </section>
  );
}
...
export default async function SimPlayersPage({ searchParams }: PageProps) {
  ...
  const rows: MatchupRow[] = await Promise.all(games.map(async (game) => ({ game, projection: withCalibratedNbaPlayers(await buildSimProjection({ ...game, simulationRuns: boardRuns })) })));
  ...
  return (
    <div className="space-y-6">
      ...
      <section className="grid gap-3 md:grid-cols-4"><SimMetricTile label="Matchups" value={String(displayedRows.length)} sub={`${activeLeague} active slate`} /><SimMetricTile label="Player rows" value={String(playerRows)} sub="Projected box-score entries" /><SimMetricTile label="Attack / Watch" value={`${attack} / ${watch}`} sub="NBA governor tiers" emphasis="strong" /><SimMetricTile label="Runs" value={boardRuns >= 1000 ? `${Math.round(boardRuns / 1000)}k` : String(boardRuns)} sub="Per-player simulation depth" /></section>
