"""Optimized Flashscore scraper with parallel polling and smart caching

Improvements over live_odds_scraper.py:
  1. Parallel sport polling (fetch all sports concurrently, not sequentially)
  2. Event deduplication + change tracking (skip unchanged events)
  3. ESPN-based event discovery (reduce Flashscore load)
  4. Adaptive timeouts per sport (faster sports = faster polls)
  5. Better bookmaker coverage (collect ALL available odds, not just best)
  6. Fallback chains (if Flashscore fails, try ESPN)
"""
...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("live_odds_optimized.log"), logging.StreamHandler(sys.stdout)],
)
logger = logging.getLogger(__name__)


def load_cache() -> dict[str, dict[str, Any]]:
    """Load cached odds to detect changes"""
    if not CACHE_ENABLED or not os.path.exists(CACHE_JSON_PATH):
        return {}
    try:
...
def save_cache(snapshot: dict[str, dict[str, Any]]) -> None:
    """Save snapshot as cache for next cycle"""
...
def has_changed(event_key: str, old: dict[str, Any], new: dict[str, Any]) -> bool:
    """Check if event odds changed (skip if unchanged)"""
    if not CACHE_ENABLED:
        return True
