...
function buildTeamFeedFromPbp(pbpRows: Row[]) {
  ...
  return Array.from(teams.values()).map((team) => ({
    teamName: team.teamName,
    offensiveRating: round(112 + team.scoreDelta / Math.max(1, team.events) * 100, 2),
    defensiveRating: 112,
    netRating: round(team.scoreDelta / Math.max(1, team.events) * 100, 2),
    pace: 99,
    recentForm: 0,
    __source: "pbpstats-free-warehouse",
    __sourceLabel: "Free NBA warehouse PBP fallback team feed",
    __sourceTier: "historical",
    __sourcePriority: 8,
    __sourceWeight: 0.82,
    __license: "public-or-self-hosted"
...
function buildPlayerFeed(playerBoxRows: Row[]) {
  ...
  return Array.from(players.values()).map((player) => {
    ...
    return {
      ...
      games: player.games,
      minutes: round(player.minutes / Math.max(1, player.games), 2),
      ...
      depthPower: round(Math.min(4, player.minutes / Math.max(1, player.games) / 8), 2),
      injuryPenalty: 0,
      fatigue: 0,
      volatility: 1.1,
...
function buildHistoryFeed(scheduleRows: Row[], teamRows: Row[]) {
  ...
  return Array.from(teams).map((teamName) => {
    ...
    return {
      teamName,
      headToHeadEdge: 0,
      recentOffense: round(recent * 0.35, 2),
      recentDefense: round(recent * 0.25, 2),
      recentShooting: 0,
      recentTurnovers: 0,
      recentRebounding: 0,
      starMatchup: 0,
...
function buildRatingFeed(teamFeed: Row[], playerFeed: Row[]) {
  ...
  for (const row of playerFeed) {
    const teamName = text(row, ["teamName", "team"]);
    if (!teamName) continue;
    const key = normalizeTeam(teamName);
    playerByTeam.set(key, [...(playerByTeam.get(key) ?? []), row]);
  }
  return teamFeed.map((team) => {
    ...
    return {
      teamName,
      overall: round(78 + net * 1.4, 2),
      offense: round(offense, 2),
      defense: round(defense, 2),
      shooting: round(70 + (num(team, ["threePointAccuracy"], 35) - 32) * 2, 2),
...
function main() {
  ...
  const historyFeed = buildHistoryFeed(schedule.rows, teamFeed);
  const ratingFeed = buildRatingFeed(teamFeed, playerFeed);

  const written = {
    team: writeFeed(outDir, "team", teamFeed),
    player: writeFeed(outDir, "player", playerFeed),
    history: writeFeed(outDir, "history", historyFeed),
    rating: writeFeed(outDir, "rating", ratingFeed)
  };
  ...
  console.log(JSON.stringify({
    ...
    outDir,
    sources: {
      schedule: { file: schedule.filePath, rows: schedule.rows.length },
      teamBox: { file: teamBox.filePath, rows: teamBox.rows.length },
      playerBox: { file: playerBox.filePath, rows: playerBox.rows.length },
      pbp: { file: pbp.filePath, rows: pbp.rows.length }
    },
    outputRows: {
      team: teamFeed.length,
      player: playerFeed.length,
      history: historyFeed.length,
      rating: ratingFeed.length
    },
    written
  }, null, 2));
}
