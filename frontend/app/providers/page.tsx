...
type ProvidersReport = Awaited<ReturnType<typeof getLiveOddsReadinessReport>> | null;

async function getSafeLiveOddsReadinessReport(): Promise<ProvidersReport> {
  try {
    return await getLiveOddsReadinessReport();
  } catch {
    return null;
  }
}
...
export default async function ProvidersReadinessPage() {
  const report = await getSafeLiveOddsReadinessReport();
      ...
      <div className="grid gap-6">
          ...
          <div className="grid gap-5 px-6 py-6 md:px-8">
            <div className="grid gap-3">
              ...
              <h1 className="max-w-4xl font-display text-3xl font-semibold tracking-tight text-white md:text-4xl">
                Provider diagnostics are temporarily unavailable.
              </h1>
              <p className="max-w-3xl text-sm leading-7 text-slate-300 md:text-base">
                The page stays online, but the readiness service did not return a
                report. Backend health, winner selection, and book-feed diagnostics
                will repopulate when the provider stack responds cleanly again.
              </p>
            </div>
          </div>
        </Card>
        ...
        <Card className="grid gap-4 p-5">
          ...
          <div className="rounded-2xl border border-amber-400/20 bg-amber-500/10 px-4 py-3 text-sm text-amber-100">
            No readiness report is available right now.
          </div>
        </Card>
      </div>
    );
      ...
      <Card className="overflow-hidden border border-white/10 bg-[radial-gradient(circle_at_top_left,_rgba(56,189,248,0.18),_transparent_28%),linear-gradient(145deg,rgba(2,6,23,0.98),rgba(15,23,42,0.94))] p-0 shadow-[0_28px_90px_rgba(2,6,23,0.42)]">
        <div className="grid gap-5 px-6 py-6 md:px-8 lg:grid-cols-[minmax(0,1.25fr)_320px] lg:items-end">
          <div className="grid gap-3">
            ...
            <h1 className="max-w-4xl font-display text-3xl font-semibold tracking-tight text-white md:text-4xl">
              One screen for backend health, book-feed wiring, and who wins board
              selection right now.
            </h1>
            <p className="max-w-3xl text-sm leading-7 text-slate-300 md:text-base">
              This page stays tied to the real SharkEdge provider stack. It does not
              pretend direct DK/FD scraping exists when only worker feed scaffolds or
              backend aggregation are live.
            </p>
          </div>
        ...
        <div className="grid gap-6">
          <Card className="grid gap-4 p-5">
            <div className="flex items-center justify-between gap-3">
              ...
              <StatusPill
                state={report.selectedBoardProvider.providerKey ? "READY" : "ERROR"}
              />
            </div>
                  ...
                  <div className="mt-4 grid gap-2 text-sm text-slate-300">
                    <div>
                      ...
                      {provider.generatedAt
                        ? new Date(provider.generatedAt).toLocaleString()
                        : "n/a"}
                    </div>
          ...
          <Card className="grid gap-4 p-5">
            <div className="flex items-center justify-between gap-3">
              ...
              <div className="text-sm text-slate-400">
                DraftKings / FanDuel direct-feed scaffolds
              </div>
            </div>
                ...
                >
                  ...
                  <div className="mt-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
                    ...
                    <div className="metric-tile">
                      ...
                      <div className="mt-2 text-sm text-white">
                        {feed.nextAllowedAt
                          ? new Date(feed.nextAllowedAt).toLocaleString()
                          : "n/a"}
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="mt-4 grid gap-2 text-sm text-slate-300">
                    ...
                    <div>
                      Warnings:{" "}
                      {feed.warnings.length ? feed.warnings.join(" | ") : "none"}
                    </div>
                  </div>
                </div>
              ))}
          ...
          <Card className="grid gap-4 p-5">
            ...
            <div className="rounded-[24px] border border-white/10 bg-slate-950/60 p-4">
              ...
              <div className="mt-3 text-sm leading-7 text-slate-300">
                {report.selectedBoardProvider.reason}
              </div>
              <div className="mt-3 text-sm text-slate-400">
                Score: {report.selectedBoardProvider.score ?? "n/a"}
              </div>
            </div>
          </Card>
