    ...
    <div className="grid gap-4">
      ...
      <section className="mobile-surface !pb-2">
        <div className="flex items-center justify-between gap-3">
          <div className="rounded-full border border-white/8 bg-[#0c1320] px-3 py-1 text-[11px] font-semibold tracking-[0.18em] text-slate-300">SHARKEDGE</div>
          <div className="rounded-full border border-[#2dd36f]/20 bg-[#2dd36f]/10 px-3 py-1 text-[11px] uppercase tracking-[0.18em] text-[#2dd36f]">{board.boardData.source}</div>
        </div>
        <div className="mt-4">
          <SectionTabs items={[{ label: 'ALL', active: board.selectedLeague === 'ALL' }, { label: 'NBA' }, { label: 'MLB' }, { label: 'NHL' }]} />
        </div>
        <div className="mt-4 grid grid-cols-5 gap-2 text-center text-[11px] uppercase tracking-[0.14em] text-slate-500">
          {['All Tiers','Tier A','Tier B','Tier C','Tier D'].map((item) => (
            <div key={item} className="rounded-[14px] border border-white/8 bg-white/[0.03] px-2 py-3">{item}</div>
          ))}
        </div>
        <div className="mt-3 flex flex-wrap gap-2 text-[11px] uppercase tracking-[0.14em] text-slate-400">
          {['Shark', '+EV', 'Time', '+EV only', 'Kalshi'].map((item, index) => (
            <div key={item} className={index === 0 ? 'rounded-full bg-white px-3 py-1 text-slate-950' : 'rounded-full border border-white/8 px-3 py-1'}>{item}</div>
          ))}
        </div>
      </section>
      ...
      <section className="grid gap-3">
        ...
        {!board.verifiedGames.length ? (
          <div className="mobile-surface text-sm leading-6 text-slate-400">{board.boardData.liveMessage ?? board.boardData.sourceNote}</div>
        ) : null}
      </section>
    </div>
  );
