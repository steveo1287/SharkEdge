...
export function SetupStateCard({ title, detail, steps }: SetupStateCardProps) {
  return (
    <Card className="border-amber-400/20 bg-amber-500/5 p-5">
      ...
      <div className="mt-4 rounded-2xl border border-line bg-slate-950/70 px-4 py-3 text-sm text-slate-400">
        SharkEdge will not backfill placeholder ledger, performance, or trend numbers while this runtime dependency is missing.
      </div>
