...
export function PropList({ props, support, spotlightPropId = null }: PropListProps) {
  if (!props.length) {
    return (
      <EmptyState
  ...
  const spotlightIndex = spotlightPropId
    ? rankedProps.findIndex(({ prop }) => prop.id === spotlightPropId)
    : -1;

  if (spotlightIndex > 0) {
    const [spotlightEntry] = rankedProps.splice(spotlightIndex, 1);
    rankedProps.unshift(spotlightEntry);
  }
      ...
      <div className="grid gap-4 xl:grid-cols-3">
        {featuredProps.map((prop) => (
          <FeaturedPropCard
            key={prop.id}
            prop={prop}
            spotlight={Boolean(spotlightPropId && prop.id === spotlightPropId)}
          />
        ))}
      </div>
        ...
        <Card className="surface-panel p-5">
          ...
          <div className="mt-4 grid gap-3">
            {restProps.map(({ prop, opportunity }) => {
              const matchupHref = prop.gameHref ?? `/game/${prop.gameId}`;
              const trapLine = getOpportunityTrapLine(opportunity);
              const spotlight = Boolean(spotlightPropId && prop.id === spotlightPropId);
              ...
              return (
                <div
                  ...
                  className={
                    spotlight
                      ? "flex flex-wrap items-center justify-between gap-4 rounded-[1.15rem] border border-sky-400/20 bg-sky-500/10 px-4 py-4"
                      : "flex flex-wrap items-center justify-between gap-4 rounded-[1.15rem] border border-white/8 bg-slate-950/60 px-4 py-4"
                  }
                >
