...
function buildMlbLane(report: Awaited<ReturnType<typeof getMlbDataQualityReport>> | null, error: string | null): DataTowerLane {
  ...
  if (!report) {
    return {
      ...
      score: 0,
      generatedAt,
      blockers: [error ?? "MLB data quality report failed."],
      warnings: [],
      metrics: [],
      recommendation: "Do not promote MLB official plays until the MLB data-quality service is returning successfully."
    };
  }
  ...
  return {
    ...
    recommendation: blockers.length
      ? "MLB should stay WATCH/PASS only until freshness and coverage blockers clear."
      : score >= 85
        ? "MLB data is strong enough for official promotion gates."
        : "MLB can run, but official PLAYs should require extra confirmation."
  };
}
...
function buildOddsLane(report: Awaited<ReturnType<typeof getLiveOddsReadinessReport>> | null, error: string | null): DataTowerLane {
  ...
  if (!report) {
    return {
      ...
      score: 0,
      generatedAt,
      blockers: [error ?? "Live odds readiness report failed."],
      warnings: [],
      metrics: [],
      recommendation: "Do not promote official plays without live odds readiness."
    };
  }
  ...
  const score = clampScore(
    (report.overallState === "READY" ? 82 : report.overallState === "DEGRADED" ? 62 : 25)
    + (boardReady ? 8 : 0)
    + Math.min(8, boardGames)
    + Math.min(7, readyFeeds * 3)
    - (bestFreshness != null && bestFreshness > 15 ? 8 : 0)
  );
  ...
  return {
    ...
    recommendation: blockers.length
      ? "Live odds are not strong enough for official promotion. Use projection/watch only."
      : score >= 85
        ? "Live odds are strong enough for official promotion gates."
        : "Live odds are usable but should remain under caution."
  };
}
...
function buildMmaLane(report: Awaited<ReturnType<typeof getUfcOperationalStatus>> | null, error: string | null): DataTowerLane {
  ...
  if (!report) {
    return {
      ...
      score: 0,
      generatedAt,
      blockers: [error ?? "MMA operational status failed."],
      warnings: [],
      metrics: [],
      recommendation: "Do not promote MMA official plays until operational status returns successfully."
    };
  }
  ...
  if (!oddsProvider?.ready) blockers.push("MMA odds provider is not ready.");
  ...
  const score = clampScore(
    30
    + (oddsProvider?.ready ? 18 : 0)
    + Math.min(16, report.predictionCount)
    + Math.min(16, resolved)
    + (calibration ? 12 : 0)
    + (predictionAge != null && predictionAge <= 60 ? 8 : 0)
    - (pending > resolved * 2 ? 6 : 0)
  );
  ...
  return {
    ...
    warnings: Array.from(new Set(warnings)),
    metrics: [
      { key: "odds_provider", label: "Odds provider ready", value: oddsProvider?.ready ? "yes" : "no", status: oddsProvider?.ready ? "USABLE" : "BLOCKED" },
    ...
    recommendation: blockers.length
      ? "MMA should stay shadow/lean only until provider and freshness blockers clear."
      : score >= 85
        ? "MMA data is strong enough for official promotion gates."
        : "MMA can run, but official PLAYs should require extra sample and CLV checks."
  };
}
...
export async function getDataControlTowerReport(): Promise<DataControlTowerReport> {
  const generatedAt = new Date().toISOString();
  const [mlbResult, oddsResult, mmaResult] = await Promise.allSettled([
    getMlbDataQualityReport({ lookbackDays: 7 }),
    getLiveOddsReadinessReport({ leagues: ["MLB"] }),
  ...
  const blockers = Array.from(new Set(lanes.flatMap((lane) => lane.blockers.map((item) => `${lane.key}: ${item}`))));
  const warnings = Array.from(new Set(lanes.flatMap((lane) => lane.warnings.map((item) => `${lane.key}: ${item}`))));
  const score = clampScore(lanes.reduce((sum, lane) => sum + lane.score, 0) / Math.max(1, lanes.length));
  const officialPromotionAllowed = deriveDataPromotionAllowed(lanes);
  const status = statusFromScore(score, officialPromotionAllowed ? [] : blockers.length ? blockers : lanes.some((lane) => lane.status === "WEAK") ? ["One or more lanes are weak."] : []);

  const nextActions = lanes.flatMap((lane) => {
    if (lane.status === "ELITE") return [];
