...
type SimProjectionInput = { id: string; label: string; startTime: string; status: string; leagueKey: LeagueKey; leagueLabel: string; simulationRuns?: number };
...
export async function buildSimProjection(input: SimProjectionInput): Promise<SimProjection> {
  ...
  if (realityIntel) {
    ...
    const nbaPlayerProjections = input.leagueKey === "NBA" && nbaProfiles ? await simulateNbaPlayerGameProjections({ awaySummary: nbaProfiles[0], homeSummary: nbaProfiles[1], projectedTotal: realityIntel.projectedTotal, volatilityIndex: realityIntel.volatilityIndex, confidence: nbaGovernor?.confidence ?? realityIntel.confidence, seedKey: `${input.id}:${input.startTime}:${matchup.away}:${matchup.home}`, simulationRuns: input.simulationRuns }) : [];
