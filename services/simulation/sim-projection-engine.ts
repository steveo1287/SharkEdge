...
function confidenceFromScore(score: number, volatility: Volatility, upsetRisk: UpsetRisk): Confidence {
  if (score >= 68 && volatility !== "High" && upsetRisk !== "High") return "A";
  if (score <= 45 || volatility === "High" || upsetRisk === "High") return "C";
  return "B";
}
...
function buildDistribution(args: {
  ...
  const base = BASE_SCORING[args.game.leagueKey] ?? { home: 24, away: 21, variance: 6, baselineTotal: 45 };
  const runs = 100;
  const outcomes: SimOutcome[] = Array.from({ length: runs }, (_, index) => {
    const runSeed = hashString(`${args.game.id}:${index}:projection-engine`);
    const vol = (0.7 + args.varianceSeed * 0.9) * base.variance * args.volatilityMultiplier;
    const home = roundScore(args.homeScore + seededOffset(runSeed, vol), args.game.leagueKey);
    const away = roundScore(args.awayScore + seededOffset(runSeed >>> 4, vol), args.game.leagueKey);
    const total = Number((home + away).toFixed(1));
    const spreadHome = Number((home - away).toFixed(1));
    return { homeScore: home, awayScore: away, total, spreadHome, homeWin: spreadHome >= 0 };
  });

  const totals = outcomes.map((outcome) => outcome.total);
  const spreads = outcomes.map((outcome) => outcome.spreadHome);
  const homeWins = outcomes.filter((outcome) => outcome.homeWin).length;
  const blowoutLine = args.game.leagueKey === "MLB" || args.game.leagueKey === "NHL" ? 2 : 10;
  const oneScoreLine = args.game.leagueKey === "MLB" || args.game.leagueKey === "NHL" ? 1.5 : 7;

  return {
    runs,
    homeWinPct: pct(homeWins, runs),
    awayWinPct: pct(runs - homeWins, runs),
    avgHome: Number((outcomes.reduce((sum, outcome) => sum + outcome.homeScore, 0) / runs).toFixed(1)),
    avgAway: Number((outcomes.reduce((sum, outcome) => sum + outcome.awayScore, 0) / runs).toFixed(1)),
    avgTotal: Number((outcomes.reduce((sum, outcome) => sum + outcome.total, 0) / runs).toFixed(1)),
    medianTotal: median(totals),
    medianSpreadHome: median(spreads),
    homeBlowoutPct: pct(outcomes.filter((outcome) => outcome.spreadHome >= blowoutLine).length, runs),
    awayBlowoutPct: pct(outcomes.filter((outcome) => outcome.spreadHome <= -blowoutLine).length, runs),
    oneScorePct: pct(outcomes.filter((outcome) => Math.abs(outcome.spreadHome) <= oneScoreLine).length, runs),
...
function buildInsights(args: {
  projection: Omit<SimProjection, "insights">;
}): SmartInsight[] {
  ...
  if (p.nbaIntel) {
    insights.push({
      label: `${p.nbaIntel.factorStrength.toUpperCase()} NBA intel profile`,
      tone: p.nbaIntel.confidenceScore >= 61 ? "strong" : p.nbaIntel.confidenceScore <= 45 ? "avoid" : "watch",
      detail: `Model confidence ${p.nbaIntel.confidenceScore}/100 with projected home edge ${p.nbaIntel.projectedHomeEdge}.`
    });

    const topFactor = [...p.nbaIntel.factors].sort((a, b) => Math.abs(b.contribution) - Math.abs(a.contribution))[0];
    if (topFactor) {
      insights.push({
        label: topFactor.label,
        tone: topFactor.contribution >= 0 ? "strong" : "watch",
        detail: `${topFactor.explanation} Contribution: ${topFactor.contribution > 0 ? "+" : ""}${topFactor.contribution}.`
      });
    }
  }

  if ((p.distribution.homeWinPct >= 0.58 || p.distribution.awayWinPct >= 0.58) && p.confidence !== "C") {
    insights.push({ label: "Strong directional lean", tone: "strong", detail: `${p.lean} wins a clear majority of the 100-run distribution.` });
  }

  if (p.volatility === "High" || p.distribution.oneScorePct >= 0.48) {
    insights.push({ label: "Chaos / tight-game profile", tone: "watch", detail: "High volatility or tight-game frequency makes this a lower-certainty setup." });
  }

  if (p.volatility === "High" && p.confidence === "C") {
    insights.push({ label: "Avoid forcing it", tone: "avoid", detail: "Model spread is wide and confidence is low. Better as a watchlist game." });
  }

  if (!insights.length) {
    insights.push({ label: "Neutral research spot", tone: "neutral", detail: "No major edge or danger flag. Use as baseline context." });
  }

  return insights.slice(0, 4);
}

export function buildSimProjection(game: SimGame): SimProjection {
  ...
  const nbaIntel = game.leagueKey === "NBA" ? buildNbaIntel(matchup.away, matchup.home) : undefined;
  ...
  const confidence = confidenceFromScore(nbaIntel?.confidenceScore ?? 52, volatility, upsetRisk);
  const homeWinPct = Math.max(0.29, Math.min(0.78, distribution.homeWinPct));
  ...
  const modelTags = [
    "100-run sim",
    nbaIntel ? nbaIntel.modelVersion : "generic fallback model",
    `${pace} pace`,
    `${volatility} volatility`,
    `${upsetRisk} upset risk`,
    total - base.baselineTotal > base.variance * 0.4 ? "Total leans high" : total - base.baselineTotal < -base.variance * 0.4 ? "Total leans low" : "Total near baseline"
  ];
  ...
  const withoutInsights: Omit<SimProjection, "insights"> = {
    ...
    read: nbaIntel
      ? nbaIntel.notes.join(" ")
      : confidence === "A"
        ? "Cleaner model separation. The 100-run distribution supports the primary lean."
        : confidence === "B"
          ? "Usable lean. Distribution context supports continued research."
          : "Volatile setup. Treat this as a research prompt.",
    distribution,
    nbaIntel
  };
