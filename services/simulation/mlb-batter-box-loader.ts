...
const TEAM_ALIASES: Record<string, string> = {
  "CHICAGO CUBS": "CHC", CUBS: "CHC", "ST. LOUIS CARDINALS": "STL", "ST LOUIS CARDINALS": "STL", CARDINALS: "STL",
  "LOS ANGELES DODGERS": "LAD", DODGERS: "LAD", "NEW YORK YANKEES": "NYY", YANKEES: "NYY", "NEW YORK METS": "NYM", METS: "NYM",
  "BOSTON RED SOX": "BOS", "WHITE SOX": "CWS", "CHICAGO WHITE SOX": "CWS", "KANSAS CITY ROYALS": "KC", "SAN DIEGO PADRES": "SD",
  "SAN FRANCISCO GIANTS": "SF", "TAMPA BAY RAYS": "TB", "TORONTO BLUE JAYS": "TOR", "WASHINGTON NATIONALS": "WSH"
};
...
async function loadGameOptionsFromGames(): Promise<MlbBatterBoxGameOption[]> {
  const rows = await prisma.$queryRaw<RawGameOption[]>`
    ...
    WHERE (UPPER(l.key) = 'MLB' OR UPPER(l.name) LIKE '%MLB%' OR l.sport = 'BASEBALL')
...
async function loadGameOptionsFromRatings(): Promise<MlbBatterBoxGameOption[]> {
  const rows = await prisma.$queryRaw<RawGameOption[]>`
    WITH hitter_teams AS (
      ...
      SELECT team, latest_at FROM hitter_teams ORDER BY latest_at DESC, hitters DESC LIMIT 8
    )
...
async function countHitters(team: string) {
  const rows = await prisma.$queryRaw<CountRow[]>`SELECT COUNT(*) AS count FROM mlb_player_ratings WHERE team = ${team};`;
  return normalizeCount(rows[0]?.count);
}

async function countPitchers(team: string) {
  const rows = await prisma.$queryRaw<CountRow[]>`SELECT COUNT(*) AS count FROM mlb_pitcher_ratings WHERE team = ${team};`;
  return normalizeCount(rows[0]?.count);
}

async function countLineups(gameId: string, team: string) {
  const rows = await prisma.$queryRaw<CountRow[]>`SELECT COUNT(*) AS count FROM mlb_lineup_snapshots WHERE game_id = ${gameId} AND team = ${team};`;
  return normalizeCount(rows[0]?.count);
}
...
export async function loadMlbBatterBoxProjection(search: SearchParamsLike): Promise<MlbBatterBoxLoadResult> {
  ...
  if (!context.available || !context.away || !context.home) {
    return { projection: null, diagnostics, error: context.reason ?? "Roster intelligence unavailable for this Batter Box matchup." };
  }
  ...
  return { projection, diagnostics, error: null };
}
