...
function eventProjectionMetadata(eventProjection: { metadataJson: Prisma.JsonValue | null } | null | undefined) {
  if (!eventProjection?.metadataJson || typeof eventProjection.metadataJson !== "object" || Array.isArray(eventProjection.metadataJson)) {
    return {} as Record<string, any>;
  }
  return eventProjection.metadataJson as Record<string, any>;
}
...
export async function recomputeEdgeSignals(eventId?: string) {
  const events = await prisma.event.findMany({
    ...
    include: {
      ...
      playerProjections: {
        orderBy: [{ modelRun: { createdAt: "desc" } }, { id: "desc" }]
      }
    }
  });
  ...
  for (const event of events) {
    ...
    for (const marketState of event.currentMarketStates) {
        ...
        if (typeof marketState.bestHomeOddsAmerican === "number" && marketState.bestHomeBookId && typeof probabilities.home === "number") {
          ...
          if (evPercent !== null) {
            await createEdgeSignal({
              ...
              lineValue: marketState.consensusLineValue,
              offeredOddsAmerican: marketState.bestHomeOddsAmerican,
              modelProb: probabilities.home,
              noVigProb: noVig?.left ?? null,
              fairOddsAmerican: buildFairPrice(probabilities.home),
              evPercent,
              modelRunId: eventProjection.modelRunId,
              flags: ["MODEL_EDGE", ...(period === "first_5" ? ["FIRST_5"] : [])],
              metadata: {
                engine: eventMetadata.engine ?? "event-projection",
                marketFamily: "moneyline",
                period
              },
              sampleSize: 30,
              hold: noVig?.hold ?? 0.04
            });
          }
        }
        ...
        if (typeof marketState.bestAwayOddsAmerican === "number" && marketState.bestAwayBookId && typeof probabilities.away === "number") {
          ...
          if (evPercent !== null) {
            await createEdgeSignal({
              ...
              lineValue: marketState.consensusLineValue,
              offeredOddsAmerican: marketState.bestAwayOddsAmerican,
              modelProb: probabilities.away,
              noVigProb: noVig?.right ?? null,
              fairOddsAmerican: buildFairPrice(probabilities.away),
              evPercent,
              modelRunId: eventProjection.modelRunId,
              flags: ["MODEL_EDGE", ...(period === "first_5" ? ["FIRST_5"] : [])],
              metadata: {
      ...
      if (marketState.marketType === "total" && eventProjection) {
        const probabilities = estimateOverUnderProbabilities({
          mean:
            typeof periodBlock.projectedTotalRuns === "number"
              ? periodBlock.projectedTotalRuns
              : eventProjection.projectedTotal,
          line: marketState.consensusLineValue,
          stdDev: typeof periodBlock.totalStdDev === "number" ? periodBlock.totalStdDev : 1.8
        });
        const noVig = noVigFromTwoWay(marketState.bestOverOddsAmerican, marketState.bestUnderOddsAmerican);
        if (probabilities && typeof marketState.bestOverOddsAmerican === "number" && marketState.bestOverBookId) {
          const evPercent = calculateEV({
            offeredOddsAmerican: marketState.bestOverOddsAmerican,
            modelProbability: probabilities.overProb
          });
          if (evPercent !== null) {
            await createEdgeSignal({
              eventId: event.id,
              ...
              lineValue: marketState.consensusLineValue,
              offeredOddsAmerican: marketState.bestOverOddsAmerican,
              modelProb: probabilities.overProb,
              noVigProb: noVig?.left ?? null,
              fairOddsAmerican: buildFairPrice(probabilities.overProb),
              evPercent,
              modelRunId: eventProjection.modelRunId,
              flags: ["MODEL_EDGE", "TOTAL_MARKET", ...(period === "first_5" ? ["FIRST_5"] : [])],
              metadata: {
                engine: eventMetadata.engine ?? "event-projection",
                marketFamily: "total",
                period,
                pushProb: probabilities.pushProb
              },
              sampleSize: 30,
              hold: noVig?.hold ?? 0.04
            });
        ...
        if (probabilities && typeof marketState.bestUnderOddsAmerican === "number" && marketState.bestUnderBookId) {
          const evPercent = calculateEV({
            offeredOddsAmerican: marketState.bestUnderOddsAmerican,
            modelProbability: probabilities.underProb
          });
          if (evPercent !== null) {
            await createEdgeSignal({
              eventId: event.id,
              ...
              lineValue: marketState.consensusLineValue,
              offeredOddsAmerican: marketState.bestUnderOddsAmerican,
              modelProb: probabilities.underProb,
              noVigProb: noVig?.right ?? null,
              fairOddsAmerican: buildFairPrice(probabilities.underProb),
              evPercent,
              modelRunId: eventProjection.modelRunId,
              flags: ["MODEL_EDGE", "TOTAL_MARKET", ...(period === "first_5" ? ["FIRST_5"] : [])],
              metadata: {
      ...
      if (marketState.marketType === "team_total" && eventProjection && (marketState as any).selectionCompetitorId) {
        ...
        const teamMean =
          typeof (isHome ? periodBlock.projectedHomeRuns : periodBlock.projectedAwayRuns) === "number"
            ? isHome
              ? periodBlock.projectedHomeRuns
              : periodBlock.projectedAwayRuns
            : isHome
              ? eventProjection.projectedHomeScore
              : eventProjection.projectedAwayScore;
        const teamStdDev = typeof (isHome ? periodBlock.homeRunsStdDev : periodBlock.awayRunsStdDev) === "number"
          ? (isHome ? periodBlock.homeRunsStdDev : periodBlock.awayRunsStdDev)
          : 1.35;
        const probabilities = estimateOverUnderProbabilities({
          mean: teamMean,
          line: marketState.consensusLineValue,
        ...
        if (probabilities && typeof marketState.bestOverOddsAmerican === "number" && marketState.bestOverBookId) {
          ...
          if (evPercent !== null) {
            await createEdgeSignal({
              ...
              evPercent,
              modelRunId: eventProjection.modelRunId,
              flags: ["MODEL_EDGE", "TEAM_TOTAL", ...(period === "first_5" ? ["FIRST_5"] : [])],
              metadata: {
                engine: eventMetadata.engine ?? "event-projection",
                marketFamily: "team_total",
                period,
                teamSide: isHome ? "home" : "away",
                projectedRuns: teamMean,
                pushProb: probabilities.pushProb
              },
              sampleSize: 26,
              hold: noVig?.hold ?? 0.05
            });
          }
        }
        if (probabilities && typeof marketState.bestUnderOddsAmerican === "number" && marketState.bestUnderBookId) {
      ...
      if (marketState.playerId) {
        ...
        const probabilities = estimateOverUnderProbabilities({
          mean: projection.meanValue,
          line: marketState.consensusLineValue ?? projection.meanValue,
          stdDev: projection.stdDev ?? 1.2
        });
        const noVig = noVigFromTwoWay(marketState.bestOverOddsAmerican, marketState.bestUnderOddsAmerican);
        const projectionMeta =
          projection.metadataJson && typeof projection.metadataJson === "object" && !Array.isArray(projection.metadataJson)
            ? (projection.metadataJson as Record<string, any>)
            : {};
        ...
        if (probabilities && typeof marketState.bestOverOddsAmerican === "number" && marketState.bestOverBookId) {
          ...
          if (evPercent !== null) {
            await createEdgeSignal({
              ...
              marketType: marketState.marketType,
              period,
              playerId: marketState.playerId,
              sportsbookId: marketState.bestOverBookId,
              side: "over",
              lineValue: marketState.consensusLineValue ?? projection.meanValue,
              offeredOddsAmerican: marketState.bestOverOddsAmerican,
              modelProb: probabilities.overProb,
              noVigProb: noVig?.left ?? null,
              fairOddsAmerican: buildFairPrice(probabilities.overProb),
              ...
              flags: ["MODEL_EDGE", "PLAYER_PROP"],
              metadata: {
                engine: projectionMeta.engine ?? "player-projection",
                starterName: projectionMeta.starterName ?? null,
                statKey: projection.statKey,
                pushProb: probabilities.pushProb
              },
              sampleSize: Number(projectionMeta.sampleSize ?? 12),
              hold: noVig?.hold ?? 0.05
            });
          }
        }
