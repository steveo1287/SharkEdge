...
async function probeBookFeedProvider(
  ...
  if (!configured) {
    return {
      ...
      reason: `Set ${provider.key === "draftkings" ? "SHARKEDGE_DRAFTKINGS_FEED_URL" : provider.key === "fanduel" ? "SHARKEDGE_FANDUEL_FEED_URL" : "SHARKEDGE_BACKEND_URL / Oddsharvester push feed"} or SHARKEDGE_BACKEND_URL to enable this worker-only feed.`,
