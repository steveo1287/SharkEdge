...
function displayName(value: { displayName?: string } | undefined) {
...
function isFinal(game: MlbScheduleGame) {
  const status = `${game.status?.abstractGameState ?? ""} ${game.status?.detailedState ?? ""} ${game.status?.statusCode ?? ""}`.toLowerCase();
  return status.includes("final") || status.includes("completed game") || game.status?.statusCode === "F";
}
...
async function fetchTeamRecent(teamId: number, name: string, abbreviation: string, date: string): Promise<MlbTrendTeamSnapshot> {
...
async function fetchPitcher(playerId: number | undefined, name: string | undefined, season: string): Promise<MlbTrendPitcherSnapshot> {
  ...
  const seasonBlock = stats.find((block) => displayName(block.type) === "season") ?? stats[0];
  const gameLogBlock = stats.find((block) => displayName(block.type).includes("gamelog"));
...
async function hydrateGame(game: MlbScheduleGame, date: string, season: string, caches: {
  teams: Map<number, Promise<MlbTrendTeamSnapshot>>;
  pitchers: Map<number, Promise<MlbTrendPitcherSnapshot>>;
}): Promise<MlbTrendGame | null> {
  ...
  const awayTeam = caches.teams.get(awayInfo.id) ?? fetchTeamRecent(awayInfo.id, awayInfo.name, awayInfo.abbreviation, date).catch(() => emptyTeam(awayInfo.id, awayInfo.name, awayInfo.abbreviation));
  const homeTeam = caches.teams.get(homeInfo.id) ?? fetchTeamRecent(homeInfo.id, homeInfo.name, homeInfo.abbreviation, date).catch(() => emptyTeam(homeInfo.id, homeInfo.name, homeInfo.abbreviation));
  caches.teams.set(awayInfo.id, awayTeam);
  caches.teams.set(homeInfo.id, homeTeam);

  const awayPitcherId = game.teams?.away?.probablePitcher?.id;
  const homePitcherId = game.teams?.home?.probablePitcher?.id;
  const awayPitcher = awayPitcherId
    ? caches.pitchers.get(awayPitcherId) ?? fetchPitcher(awayPitcherId, game.teams?.away?.probablePitcher?.fullName, season).catch(() => emptyPitcher(awayPitcherId, game.teams?.away?.probablePitcher?.fullName ?? "TBD"))
    : Promise.resolve(emptyPitcher(null, game.teams?.away?.probablePitcher?.fullName ?? "TBD"));
  const homePitcher = homePitcherId
    ? caches.pitchers.get(homePitcherId) ?? fetchPitcher(homePitcherId, game.teams?.home?.probablePitcher?.fullName, season).catch(() => emptyPitcher(homePitcherId, game.teams?.home?.probablePitcher?.fullName ?? "TBD"))
    : Promise.resolve(emptyPitcher(null, game.teams?.home?.probablePitcher?.fullName ?? "TBD"));
  if (awayPitcherId) caches.pitchers.set(awayPitcherId, awayPitcher);
  if (homePitcherId) caches.pitchers.set(homePitcherId, homePitcher);
  ...
  return {
    ...
    away,
    home,
    awayPitcher: awayPitcherSnapshot,
    homePitcher: homePitcherSnapshot
  };
}
...
export async function buildMlbStatBackedTrends(args: { date?: string } = {}): Promise<MlbStatTrendsPayload> {
  ...
  try {
    ...
    const games = (await Promise.all(schedule.map((game) => hydrateGame(game, date, season, caches)))).filter((game): game is MlbTrendGame => Boolean(game));
    const trends = games
      .flatMap((game) => [buildStarterTrend(game), buildFormTrend(game), buildTotalTrend(game)])
      .filter((trend): trend is MlbStatTrend => Boolean(trend))
      .sort((left, right) => {
