...
export async function getOddsApiBudget(): Promise<BudgetState> {
  ...
  await writeHotCache(key, fresh, BUDGET_CACHE_TTL_SECONDS);
  return fresh;
}
...
async function incrementBudget(amount: number) {
...
function parseSports(input?: string | null) {
  if (!input) return DEFAULT_SPORTS;
  return input
    .split(",")
...
function canPull(budget: BudgetState, requestedPulls: number, mode: PullMode) {
...
async function fetchSportOdds(sport: string, apiKey: string) {
...
export async function readLatestOddsApiSnapshot() {
  return readHotCache<OddsApiSnapshot>(SNAPSHOT_CACHE_KEY);
}

export async function runOddsApiSnapshotPull(args?: {
  mode?: PullMode;
  sportsCsv?: string | null;
}) {
  const mode = args?.mode ?? "regular";
  const sports = parseSports(args?.sportsCsv);
  ...
  if (!apiKey) {
    return {
      ok: false,
      skipped: true,
      reason: "THE_ODDS_API_KEY is not configured in the environment.",
      budget: await getOddsApiBudget(),
      snapshot: await readLatestOddsApiSnapshot()
    };
  }

  const budget = await getOddsApiBudget();
  const requestedPulls = sports.length;
  const allowed = canPull(budget, requestedPulls, mode);
  if (!allowed.ok) {
    return {
      ok: false,
      skipped: true,
  ...
  const fetched = await Promise.all(
    sports.map(async (sport) => ({ sport, events: await fetchSportOdds(sport, apiKey) }))
  );

  const nextBudget = await incrementBudget(requestedPulls);
  const snapshot: OddsApiSnapshot = {
    meta: {
      provider: PROVIDER,
      generatedAt: new Date().toISOString(),
      mode,
      sports,
      requestsUsed: requestedPulls,
      monthlyUsed: nextBudget.used,
      monthlyLimit: nextBudget.limit
    },
    events: fetched.flatMap((entry) =>
      Array.isArray(entry.events)
        ? entry.events.map((event) => ({ ...(event as object), sport_key: entry.sport }))
        : []
    )
  };

  await writeHotCache(SNAPSHOT_CACHE_KEY, snapshot, SNAPSHOT_CACHE_TTL_SECONDS);
