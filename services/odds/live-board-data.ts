...
function findBookOutcome(
  ...
  return (
    bookmaker.markets[marketType].find((outcome: CurrentOddsBookOutcome) => {
...
function buildLiveMarketView(
  ...
  return (
    buildBookSpecificMarketView(leagueKey, game, marketType, sportsbookKey) ??
...
function resolveLeagueKeyForSport(sport: CurrentOddsSport) {
  return getLeagueForSportKey(sport.key) ?? getLeagueForSportKey(sport.short_title) ?? getLeagueForSportKey(sport.title);
}
...
function getLiveSourceNote(response: CurrentOddsBoardResponse) {
  const providerLabel = response.provider === "oddsharvester" ? "OddsHarvester" : "the live backend";

  if (response.errors.length) {
    return `${providerLabel} is connected for the board, with partial fetch warnings still reported by the backend.`;
  }

  return `${providerLabel} is powering the live current board.`;
}
