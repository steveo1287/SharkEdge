...
const IMPORT_LEAGUE_CONFIG = {
  NBA: LEAGUE_CONFIG.NBA,
  NCAAB: LEAGUE_CONFIG.NCAAB,
  MLB: {
    sportKey: "baseball_mlb",
    leagueName: "Major League Baseball",
    sportKeyName: "baseball",
    sportName: "Baseball",
    sportCode: "BASEBALL" as const
  },
  NFL: {
    sportKey: "americanfootball_nfl",
    leagueName: "National Football League",
    sportKeyName: "football",
    sportName: "Football",
...
type SupportedImportedPropLeagueKey = keyof typeof IMPORT_LEAGUE_CONFIG;
...
type DkScrapyImportArgs = {
  path: string;
  dryRun?: boolean;
  league?: SupportedImportedPropLeagueKey | "ALL";
};

type DkScrapyImportRow = {
  sport?: string | null;
  league?: string | null;
...
function decimalToAmerican(value: number) {
...
function normalizeOddsToAmerican(value: number | string | null | undefined) {
  ...
  return decimalToAmerican(numeric);
}

function parseGto76CapturedAt(value: number | string | null | undefined) {
  const numeric = toNumber(value);
  if (numeric === null) {
    return new Date();
  }
...
function parseImportedDateTime(value: string | null | undefined, fallback: Date) {
  ...
  return buildDateTimeFromClock(raw, fallback);
}

function buildDateTimeFromClock(clockValue: string | null | undefined, fallback: Date) {
  const trimmed = clockValue?.trim();
  if (!trimmed) {
    return fallback;
  }
...
function resolveImportedLeagueKey(
  rawValue: string | null | undefined
): SupportedImportedPropLeagueKey | null {
  const normalized = normalizeToken(rawValue);

  if (normalized === "nba" || normalized === "basketballnba") return "NBA";
  if (
    normalized === "ncaab" ||
    normalized === "basketballncaab" ||
    normalized === "menscollegebasketball"
  ) {
    return "NCAAB";
  }
  if (normalized === "mlb" || normalized === "baseballmlb") return "MLB";
  if (normalized === "nfl" || normalized === "americanfootballnfl") return "NFL";
  if (
    normalized === "ncaaf" ||
    normalized === "americanfootballncaaf" ||
...
function normalizeImportedMarketType(
  value: string | null | undefined
): PropMarketType | "team_total" | "spread" | "moneyline" | "total" | null {
  const normalized = normalizeToken(value);
  ...
  if (
    normalized.includes("playerpoints") ||
    normalized === "points" ||
    normalized === "pointstotal"
  ) {
    return "player_points";
  }
  if (normalized.includes("playerrebounds") || normalized === "rebounds") {
    return "player_rebounds";
  }
  if (normalized.includes("playerassists") || normalized === "assists") {
    return "player_assists";
  }
  if (normalized.includes("playerthrees") || normalized.includes("3pm") || normalized.includes("threes")) {
    return "player_threes";
  }
  if (normalized.includes("pitcherouts")) {
    return "player_pitcher_outs";
  }
  if (normalized.includes("pitcherstrikeouts") || normalized.includes("strikeouts")) {
    return "player_pitcher_strikeouts";
  }
  if (normalized.includes("teamtotal")) {
    return "team_total";
  }
  if (normalized === "spread" || normalized.includes("gamespread")) {
    return "spread";
  }
  if (normalized === "moneyline" || normalized.includes("matchwinner")) {
    return "moneyline";
  }
  if (normalized === "total" || normalized.includes("gametotal")) {
    return "total";
  }
...
async function ensureImportedLeagueAndSport(leagueKey: SupportedImportedPropLeagueKey) {
  const config = IMPORT_LEAGUE_CONFIG[leagueKey];
...
async function loadRowsFromFile<T>(filePath: string) {
  ...
  if (extension === ".json") {
    ...
    throw new Error("JSON import must be an array or an object with rows/items.");
  }
  ...
  throw new Error("Unsupported import format. Use .json or .csv exports.");
}

async function findOrCreateGto76Game(args: {
  leagueId: string;
  teamId: string;
  teamName: string;
  startTime: Date;
  ...
  const candidates = await prisma.game.findMany({
    where: {
      ...
      startTime: {
        gte: new Date(args.startTime.getTime() - 6 * 60 * 60 * 1000),
        lte: new Date(args.startTime.getTime() + 6 * 60 * 60 * 1000)
      }
    },
    orderBy: {
      startTime: "asc"
  ...
  if (candidates.length) {
    return [...candidates].sort((left, right) => {
      const leftDelta = Math.abs(left.startTime.getTime() - args.startTime.getTime());
      const rightDelta = Math.abs(right.startTime.getTime() - args.startTime.getTime());
      return leftDelta - rightDelta;
    })[0];
  }

  const opponent = await ensureTeam(args.leagueId, "Opponent TBD", "gto76_bets_placeholder");
  return ensureGame({
    leagueId: args.leagueId,
    eventId: `gto76:${args.teamId}:${args.startTime.toISOString()}`,
    startTime: args.startTime.toISOString(),
    homeTeamId: args.teamId,
    awayTeamId: opponent.id
  });
}

async function findOrCreateImportedGame(args: {
  leagueId: string;
  source: string;
  eventId?: string | null;
  startTime: Date;
  ...
  const homeName = args.homeTeamName?.trim() || args.fallbackTeamName?.trim() || "Home TBD";
  const awayName = args.awayTeamName?.trim() || "Away TBD";
  const homeTeam = await ensureTeam(args.leagueId, homeName, args.source);
  const awayTeam =
    normalizeText(homeName) === normalizeText(awayName)
      ? await ensureTeam(args.leagueId, "Opponent TBD", `${args.source}_placeholder`)
      : await ensureTeam(args.leagueId, awayName, args.source);

  return ensureGame({
    leagueId: args.leagueId,
    eventId:
      args.eventId?.trim() ||
      `${args.source}:${normalizeToken(homeName)}:${normalizeToken(awayName)}:${args.startTime.toISOString()}`,
    startTime: args.startTime.toISOString(),
...
async function upsertMarketAndSnapshot(args: {
  ...
  marketType: PropMarketType | "team_total" | "spread" | "moneyline" | "total";
  side: string;
  line: number | null;
  price: number;
  capturedAt: Date;
...
export async function importGto76Props(
  args: Gto76ImportArgs
): Promise<Gto76ImportResult> {
  const rows = await loadRowsFromFile<Gto76ImportRow>(args.path);
  const warnings: string[] = [];
  let storedRows = 0;
  let storedSnapshots = 0;
  let skippedRows = 0;
  ...
  const { league } = await ensureLeagueAndSport("NBA");

  for (const row of rows) {
    try {
      const playerName = row.player_name?.trim();
      const teamName = row.teams?.trim();
      const line = toNumber(row.player_total);
      const overPrice = normalizeOddsToAmerican(row.over);
      const underPrice = normalizeOddsToAmerican(row.under);
      const bookName = row.bookie_name?.trim();

      if (!playerName || !teamName || line === null || !bookName) {
        skippedRows += 1;
        warnings.push("Skipped a gto76 row because player, team, book, or line was missing.");
        continue;
      }

      if (overPrice === null && underPrice === null) {
        skippedRows += 1;
        warnings.push(`Skipped ${playerName} because both over/under prices were unusable.`);
        continue;
      }

      const capturedAt = parseGto76CapturedAt(row.date_time);
      const eventStart = buildDateTimeFromClock(row.start_time, capturedAt);
      const team = await ensureTeam(league.id, teamName, "gto76_bets");
      const game = await findOrCreateGto76Game({
        leagueId: league.id,
        teamId: team.id,
        teamName: team.name,
        startTime: eventStart
      });
      const sportsbook = await ensureSportsbook(
        normalizeToken(bookName) || "gto76_bets",
        bookName
      );
      const player = await ensurePlayer({
        leagueId: league.id,
      ...
      ].filter((entry): entry is { side: "over" | "under"; price: number } => entry.price !== null);
      ...
      skippedRows += 1;
      warnings.push(
        `Skipped ${row.player_name ?? "unknown player"} from gto76 import: ${
...
export async function importDKscraPyProps(
  args: DkScrapyImportArgs
): Promise<DkScrapyImportResult> {
  const rows = await loadRowsFromFile<DkScrapyImportRow>(args.path);
  const warnings: string[] = [];
  let storedRows = 0;
  let storedSnapshots = 0;
  let skippedRows = 0;
  ...
  for (const row of rows) {
    try {
      const requestedLeague =
        args.league && args.league !== "ALL" ? args.league : null;
      const leagueKey =
        requestedLeague ??
        resolveImportedLeagueKey(row.league) ??
        resolveImportedLeagueKey(row.sport);

      if (!leagueKey) {
        skippedRows += 1;
        warnings.push("Skipped a DKscraPy row because no supported league could be inferred.");
        continue;
      }

      const marketType =
        normalizeImportedMarketType(row.market_type) ??
        normalizeImportedMarketType(row.market) ??
        normalizeImportedMarketType(row.bet_type);

      if (!marketType) {
        skippedRows += 1;
        warnings.push(
          `Skipped ${row.player_name ?? row.player ?? row.team_name ?? row.team ?? "unknown selection"} because the market type was unsupported.`
        );
        continue;
      }

      const line = toNumber(row.line ?? row.point);
      const explicitPrice = normalizeOddsToAmerican(row.price ?? row.odds);
      const overPrice = normalizeOddsToAmerican(row.over_odds ?? row.over);
      const underPrice = normalizeOddsToAmerican(row.under_odds ?? row.under);
      const side = normalizeImportedSide(row.side ?? row.selection);
      const bookmakerName =
        row.bookmaker?.trim() ||
        row.book?.trim() ||
        row.sportsbook?.trim() ||
      ...
      const capturedAt = parseGto76CapturedAt(row.date_time);
      const startTime = parseImportedDateTime(
        row.commence_time ?? row.game_time ?? row.start_time,
        capturedAt
      );
      const { league } = await ensureImportedLeagueAndSport(leagueKey);
      const game = await findOrCreateImportedGame({
        leagueId: league.id,
        source: "dkscrapy",
        eventId: row.event_id ?? null,
        startTime,
        homeTeamName: row.home_team ?? row.team_name ?? row.team ?? null,
        awayTeamName: row.away_team ?? null,
        fallbackTeamName: row.team_name ?? row.team ?? null
      });
      const sportsbook = await ensureSportsbook(
        normalizeToken(bookmakerName) || "dkscrapy",
        bookmakerName
      );

      const selectionTeamName =
        row.team_name?.trim() ||
        row.team?.trim() ||
        row.home_team?.trim() ||
        null;
      const selectionTeam = selectionTeamName
        ? await ensureTeam(league.id, selectionTeamName, "dkscrapy")
        : null;
      const playerName = row.player_name?.trim() || row.player?.trim() || null;
      const player =
        playerName && selectionTeam
          ? await ensurePlayer({
              leagueId: league.id,
              homeTeamId: game.homeTeamId,
              awayTeamId: game.awayTeamId,
              defaultTeamId: selectionTeam.id,
              playerName,
              source: "dkscrapy"
            })
          : null;
      ...
      const marketRows =
        explicitPrice !== null && side
          ? [{ side, price: explicitPrice }]
          : [
              { side: "over", price: overPrice },
              { side: "under", price: underPrice }
            ].filter(
              (entry): entry is { side: string; price: number } => entry.price !== null
            );

      if (!marketRows.length) {
        skippedRows += 1;
        warnings.push(
          `Skipped ${playerName ?? selectionTeamName ?? "unknown selection"} because no usable odds were found.`
        );
        continue;
      }
      ...
      skippedRows += 1;
      warnings.push(
        `Skipped ${row.player_name ?? row.player ?? row.team_name ?? row.team ?? "unknown selection"} from DKscraPy import: ${
          error instanceof Error ? error.message : "unknown import error"
        }`
      );
    }
