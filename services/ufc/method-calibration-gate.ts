...
function weakerConfidence(left: UfcPromotionGate["confidenceCap"], right: UfcPromotionGate["confidenceCap"]) { return confidenceRank(left) <= confidenceRank(right) ? left : right; }
...
function weakerGrade(left: UfcPromotionGate["grade"], right: UfcPromotionGate["grade"]) { return gradeRank(left) <= gradeRank(right) ? left : right; }
...
export async function getUfcMethodCalibration(modelVersion: string, options: { limit?: number } = {}): Promise<UfcMethodCalibration> {
  ...
  const actualRates = sampleSize > 0 ? { KO_TKO: round(actual.KO_TKO / sampleSize), SUBMISSION: round(actual.SUBMISSION / sampleSize), DECISION: round(actual.DECISION / sampleSize) } : { KO_TKO: 0.33, SUBMISSION: 0.22, DECISION: 0.45 };
  const predictedAverages = sampleSize > 0 ? normalize({ KO_TKO: predicted.KO_TKO / sampleSize, SUBMISSION: predicted.SUBMISSION / sampleSize, DECISION: predicted.DECISION / sampleSize }) : { KO_TKO: 0.33, SUBMISSION: 0.22, DECISION: 0.45 };
...
export function applyUfcMethodCalibration(methods: UfcMethodProbabilities, calibration: UfcMethodCalibration): UfcMethodProbabilities {
  ...
  return normalize({ KO_TKO: methods.KO_TKO + calibration.corrections.KO_TKO, SUBMISSION: methods.SUBMISSION + calibration.corrections.SUBMISSION, DECISION: methods.DECISION + calibration.corrections.DECISION });
}
...
export function buildUfcPromotionGate(input: {
  ...
  if (gradeRank(input.dataQualityGrade) <= 1) {
    ...
    grade = weakerGrade(grade, "C");
    confidenceCap = weakerConfidence(confidenceCap, "MEDIUM");
    reasons.push("Data quality C: limited profile depth.");
  }

  if (input.noMarketEdge) {
    grade = "D";
    confidenceCap = weakerConfidence(confidenceCap, "LOW");
    reasons.push(input.hasRealMarket ? "No promotable market edge after data/uncertainty gating." : "NO MARKET EDGE: no real two-sided market price available.");
  }

  if (input.uncertaintyCrossesMarket) {
    grade = weakerGrade(grade, "C");
    confidenceCap = weakerConfidence(confidenceCap, "MEDIUM");
    reasons.push("Confidence band crosses market-implied probability; edge is not clean enough to promote.");
  }

  if (typeof input.confidenceBandWidth === "number" && input.confidenceBandWidth >= 0.14) {
    grade = weakerGrade(grade, "C");
    confidenceCap = weakerConfidence(confidenceCap, "MEDIUM");
    reasons.push(`Uncertainty band is wide (${Math.round(input.confidenceBandWidth * 1000) / 10} pts).`);
  }
  ...
  if (!input.hasPriorSignal) {
    grade = weakerGrade(grade, "B");
    reasons.push("No enriched fighter-background prior applied.");
  }
  ...
  for (const code of marketReasons) {
    if (code === "NO_REAL_MARKET" || code === "DATA_QUALITY_D" || code === "COLD_START_ACTIVE") continue;
    if (reasons.length < 12) reasons.push(`Market/uncertainty flag: ${code}.`);
  }
  ...
  const status = grade === "D" || finalConfidence === "LOW"
    ? "SHADOW_ONLY"
    : grade === "A" && finalConfidence !== "MEDIUM" && input.edgePct != null && input.edgePct >= 1.5 && input.pickProbability >= 0.555 && !input.uncertaintyCrossesMarket
      ? "PROMOTABLE"
      : "WATCHLIST";
