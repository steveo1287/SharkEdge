...
function simOne(args: { a: UfcFighterSkillProfile; b: UfcFighterSkillProfile; d: UfcMatchupSkillDeltas; rounds: 3 | 5; random: () => number }) {
  ...
  for (let roundNo = 1; roundNo <= rounds; roundNo += 1) {
    const lateA = roundNo >= 3 ? (a.cardio.round3 - 50) / 100 : 0;
    const lateB = roundNo >= 3 ? (b.cardio.round3 - 50) / 100 : 0;
    ...
    for (let exchange = 0; exchange < exchanges; exchange += 1) {
      ...
      if (takedownAttempt) {
        ...
        if (success) {
          ...
          if (random() < trans.groundControlToSubmissionThreatA && random() < trans.finishAttemptToSubmissionA + (roundNo >= 3 ? Math.max(0, -lateB) * 0.03 : 0)) {
      ...
      const strikeWinner = random() < clamp(0.5 + (initiator === "A" ? d.strikingEdgeA : -d.strikingEdgeA) * 0.38 + (initiator === "A" ? lateA - lateB : lateB - lateA) * 0.08, 0.16, 0.84) ? initiator : initiator === "A" ? "B" : "A";
...
function pathSummary(a: UfcFighterSkillProfile, b: UfcFighterSkillProfile, d: UfcMatchupSkillDeltas) {
  ...
  return reasons.length ? reasons.slice(0, 5) : ["Skill deltas are narrow; outcome relies more on pace variance and decision swing."];
}
