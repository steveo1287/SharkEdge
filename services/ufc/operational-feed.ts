...
function combatSportFor(row: FeedRow) {
  const fightPayload = asRecord(row.fight_payload_json);
  const eventPayload = asRecord(row.event_payload_json);
  return stringOrNull(fightPayload.combatSport) ?? stringOrNull(fightPayload.sport) ?? stringOrNull(eventPayload.combatSport) ?? stringOrNull(eventPayload.sport);
}
function qualityForRow(row: FeedRow) {
  const fightPayload = asRecord(row.fight_payload_json);
  const storedQuality = stringOrNull(fightPayload.matchupQuality);
  const storedScore = numberOrNull(fightPayload.matchupQualityScore);
  const storedReasons = asArray(fightPayload.matchupQualityReasons);
  const evaluated = evaluateUfcMatchupQuality({
    sourceKey: row.event_source_key,
    eventName: row.event_name,
    eventLabel: row.event_label,
    fighterAName: row.fighter_a_name,
    fighterBName: row.fighter_b_name,
    combatSport: combatSportFor(row),
    sourceStatus: row.source_status
  });
  if (storedQuality === "FAKE_NAVIGATION") return { ...evaluated, status: "FAKE_NAVIGATION" as const, score: storedScore ?? evaluated.score, fakeNavigation: true, valid: false, questionable: false, reasons: storedReasons.length ? storedReasons : evaluated.reasons };
  return evaluated;
}
function isOperationalFeedEligible(row: FeedRow) { return !qualityForRow(row).fakeNavigation; }
...
function mapRows(rows: FeedRow[], limit: number): UfcOperationalFeedCard[] {
  return rows.filter(isOperationalFeedEligible).slice(0, limit).map((row) => {
    ...
    return {
      ...
      promotionKey: stringOrNull(eventPayload.promotionKey) ?? row.event_source_key,
      promotionName: stringOrNull(eventPayload.promotionName) ?? (row.event_source_key === "mvp" ? "Most Valuable Promotions" : row.event_source_key === "ufc" || row.event_source_key === "ufcstats" || row.event_source_key === "ufc.com" ? "UFC" : null),
      combatSport: combatSportFor(row),
...
export async function getUfcOperationalFeed(options: { modelVersion?: string; limit?: number; includePast?: boolean; promotionStatus?: string | null } = {}): Promise<UfcOperationalFeedCard[]> {
  ...
  const requestedLimit = Math.max(1, Math.min(200, Math.floor(options.limit ?? 100)));
  const queryLimit = Math.max(requestedLimit, Math.min(500, requestedLimit * 8));
  ...
  try { return mapRows(await queryEventLinkedFeed(modelVersion, queryLimit, includePast, promotionStatus), requestedLimit); }
  catch { try { return mapRows(await queryLegacyFeed(modelVersion, queryLimit, includePast, promotionStatus), requestedLimit); } catch { return []; } }
}
