...
import { classifyCombatSportsEnrichmentLane, type CombatSportsEnrichmentLane } from "@/services/ufc/enrichment-lane-classifier";
...
type FighterCoverage = {
  ...
  opponentId: string;
  opponentName: string;
  enrichmentLane: CombatSportsEnrichmentLane;
  ...
  fields: { proFights: number | null; ufcFights: number | null; roundsFought: number | null; slpm: number | null; sapm: number | null; takedownDefensePct: number | null };
...
export type UfcProfileCoverageReport = {
  ...
  laneCounts: CountMap;
  laneGradeCounts: Record<string, CountMap>;
  ...
  blockedFights: Array<{ fightId: string; eventLabel: string; fightDate: string; fighterA: { name: string; grade: string; score: number; lane: string } | null; fighterB: { name: string; grade: string; score: number; lane: string } | null; blocker: string }>;
...
function countBy(values: string[]) { return values.reduce<CountMap>((acc, value) => { acc[value] = (acc[value] ?? 0) + 1; return acc; }, {}); }
function nestedCount(items: FighterCoverage[]) {
  return items.reduce<Record<string, CountMap>>((acc, item) => {
    const lane = item.enrichmentLane;
    acc[lane] = acc[lane] ?? {};
    acc[lane][item.grade] = (acc[lane][item.grade] ?? 0) + 1;
    return acc;
  }, {});
}
...
function recommendedSource(row: CoverageRow, missingCritical: string[], missingUseful: string[], lane: ReturnType<typeof classifyCombatSportsEnrichmentLane>) {
  if (lane.lane !== "UFCSTATS") return lane.recommendedSource;
  if (!row.has_ufcstats_payload) return "UFCStats exact/alias profile backfill first";
...
function mapCoverage(row: CoverageRow): FighterCoverage {
  ...
  const lane = classifyCombatSportsEnrichmentLane({ fighterName: row.fighter_name, opponentName: row.opponent_name, eventLabel: row.event_label, eventName: row.event_name, sourceKey: row.source_key, existingPayload: row.fighter_payload_json });
  return {
    ...
    fields: { proFights: row.pro_fights, ufcFights: row.ufc_fights, roundsFought: row.rounds_fought, slpm: row.slpm, sapm: row.sapm, takedownDefensePct: row.takedown_defense_pct },
    missingCritical: score.missingCritical,
    missingUseful: score.missingUseful,
    recommendedNextSource: recommendedSource(row, score.missingCritical, score.missingUseful, lane)
  };
}
async function queryCoverage(modelVersion: string, horizonDays: number, limit: number) {
  return prisma.$queryRaw<CoverageRow[]>`
    WITH fight_scope AS (
      SELECT f.id, f.event_label, f.fight_date, f.fighter_a_id, f.fighter_b_id, e.event_name, e.source_key
      FROM ufc_fights f
      ...
      SELECT fs.id AS fight_id, fs.event_label, fs.event_name, fs.source_key, fs.fight_date, 'A'::text AS side, fs.fighter_a_id AS fighter_id, fs.fighter_b_id AS opponent_id FROM fight_scope fs
      UNION ALL
      SELECT fs.id AS fight_id, fs.event_label, fs.event_name, fs.source_key, fs.fight_date, 'B'::text AS side, fs.fighter_b_id AS fighter_id, fs.fighter_a_id AS opponent_id FROM fight_scope fs
    )
    SELECT s.fight_id, s.event_label, s.event_name, s.source_key, s.fight_date, s.fighter_id, ftr.full_name AS fighter_name,
      s.opponent_id, opp.full_name AS opponent_name, s.side,
      ftr.payload_json AS fighter_payload_json,
      (ftr.payload_json ? 'stats') AS has_ufcstats_payload,
      ftr.payload_json->>'ufcStatsUrl' AS ufcstats_url,
      mf.pro_fights, mf.ufc_fights, mf.rounds_fought,
...
export async function getUfcProfileCoverageReport(options: { modelVersion?: string; horizonDays?: number; limit?: number } = {}): Promise<UfcProfileCoverageReport> {
  if (!hasUsableServerDatabaseUrl()) {
    return { ok: false, modelVersion: options.modelVersion ?? "ufc-fight-iq-v1", horizonDays: options.horizonDays ?? 180, fightCount: 0, fighterSides: 0, gradeCounts: {}, laneCounts: {}, laneGradeCounts: {}, sourceCounts: {}, ufcStatsPayloadCounts: {}, coldStartCounts: {}, remainingDCount: 0, blockedFightCount: 0, blockedFights: [], worstFighters: [], coverage: [] };
  }
  ...
  const coverage = (await queryCoverage(modelVersion, horizonDays, limit)).map(mapCoverage);
  const byFight = new Map<string, FighterCoverage[]>();
  for (const item of coverage) { const list = byFight.get(item.fightId) ?? []; list.push(item); byFight.set(item.fightId, list); }
  const blockedFights = [...byFight.entries()].flatMap(([fightId, items]) => {
    ...
    return [{ fightId, eventLabel: items[0]?.eventLabel ?? fightId, fightDate: items[0]?.fightDate ?? "", fighterA: a ? { name: a.fighterName, grade: a.grade, score: a.score, lane: a.enrichmentLane } : null, fighterB: b ? { name: b.fighterName, grade: b.grade, score: b.score, lane: b.enrichmentLane } : null, blocker: weak.map((item) => `${item.fighterName}:${item.enrichmentLane}:${item.grade}/${item.score}`).join(", ") }];
  });
  const worstFighters = coverage.filter((item) => item.grade === "D" || item.score < 70).sort((a, b) => a.score - b.score || a.enrichmentLane.localeCompare(b.enrichmentLane) || a.fighterName.localeCompare(b.fighterName)).slice(0, 75);
  return {
    ok: true,
    modelVersion,
    horizonDays,
    ...
    laneCounts: countBy(coverage.map((item) => item.enrichmentLane)),
    laneGradeCounts: nestedCount(coverage),
