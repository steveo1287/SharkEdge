...
function parseReach(value: string | null) {
  if (!value) return null;
...
function labelValue(html: string, label: string) {
  ...
  const regex = new RegExp(`<i[^>]*>\\s*${escaped}:?\\s*<\\/i>\\s*([^<]+)`, "i");
  const match = html.match(regex);
  return match ? cleanHtml(match[1]) : null;
}

function parseRecord(html: string) {
  const match = html.match(/Record:\s*(\d+)\s*-\s*(\d+)\s*-\s*(\d+)/i);
  if (!match) return null;
  return { wins: Number(match[1]), losses: Number(match[2]), draws: Number(match[3]) };
}
...
async function fetchText(url: string, fetchImpl: typeof fetch) {
  const response = await fetchImpl(url, {
    headers: {
      "user-agent": "SharkEdge profile-depth backfill/1.0",
...
async function buildUfcStatsIndex(fetchImpl: typeof fetch) {
  ...
  for (const letter of letters) {
    try {
      const html = await fetchText(`http://www.ufcstats.com/statistics/fighters?char=${letter}&page=all`, fetchImpl);
      items.push(...parseIndexPage(html));
    } catch {
      // Continue; one missing letter page should not fail the whole backfill.
    }
  }
...
async function knownUpcomingFighters(limit: number, horizonDays: number) {
  return prisma.$queryRaw<KnownFighterRow[]>`
    SELECT DISTINCT ftr.id, ftr.full_name
...
function profilePayload(profile: ParsedUfcStatsProfile, fullName: string, backfilledAt: string) {
  ...
  return {
    ...
    profile: {
      ...
      reachInches: profile.reachInches,
...
export async function backfillKnownUfcFighterStats(options: { limit?: number; horizonDays?: number; dryRun?: boolean; fetchImpl?: typeof fetch } = {}): Promise<UfcStatsKnownFighterBackfillResult> {
  if (!hasUsableServerDatabaseUrl()) {
    return { ok: false, mode: options.dryRun ? "dry-run" : "backfill", requestedLimit: options.limit ?? 40, knownFighters: 0, indexedFighters: 0, matchedFighters: 0, updatedFighters: 0, roundStatsInserted: 0, unmatched: [], updated: [], errors: ["No usable server database URL is configured."] };
  }
  ...
  const fighters = await knownUpcomingFighters(limit, horizonDays);
  ...
  for (const fighter of fighters) {
    ...
    if (!indexItem) {
      unmatched.push(fighter.full_name);
      continue;
    }
    matchedFighters += 1;
    try {
      ...
      if (!hasStats && !profile.record && !profile.heightInches && !profile.reachInches) {
        unmatched.push(fighter.full_name);
        continue;
      }
  ...
  return {
    ...
    knownFighters: fighters.length,
    indexedFighters: index.size,
    matchedFighters,
    updatedFighters,
    roundStatsInserted: 0,
