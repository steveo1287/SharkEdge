...
export type UfcStatsKnownFighterBackfillResult = { ok: boolean; mode: "dry-run" | "backfill"; requestedLimit: number; offset: number; knownFighters: number; indexedFighters: number; laneSkipped: Record<string, number>; matchedFighters: number; updatedFighters: number; roundStatsInserted: number; unmatched: string[]; updated: Array<{ fighterId: string; fullName: string; matchedName: string; matchMethod: string; matchScore: number; url: string; stats: Record<string, number | null> }>; errors: string[] };
...
const NAME_ALIASES: Record<string, string[]> = { "aoriqileng": ["aori qileng"], "song yadong": ["song ya dong"], "sumudaerji": ["su mudaerji"], "sergei pavlovich": ["sergey pavlovich"], "shara magomedov": ["sharabutdin magomedov"], "zhang mingyang": ["mingyang zhang"], "yi sak lee": ["leesak yi", "lee sak yi"], "zhu kangjie": ["kangjie zhu"], "rei tsuruya": ["rei tsuruya"], "timmy cuamba": ["timothy cuamba"], "tommy gantt": ["tom gantt"], "tuco tokkos": ["tuco tokkos"], "sean o malley": ["sean o malley", "sean omalley"], "rafael fiziev": ["rafael fiziev"], "shauna bannon": ["shauna bannon"], "victor henry": ["victor henry"], "tom nolan": ["tom nolan"], "steve garcia": ["steve garcia"], "shara bullet": ["sharabutdin magomedov"] };
...
function parseIndexPage(html: string) { const rows = [...html.matchAll(/<tr[^>]*class="[^"]*b-statistics__table-row[^"]*"[^>]*>([\s\S]*?)<\/tr>/gi)].map((match) => match[1]); const fighters: FighterIndexItem[] = []; for (const row of rows) { const links = [...row.matchAll(/<a[^>]+href="([^"]+)"[^>]*>([\s\S]*?)<\/a>/gi)].map((match) => ({ url: match[1], text: cleanHtml(match[2]) })).filter((link) => link.text); if (links.length < 2) continue; const name = `${links[0].text} ${links[1].text}`.replace(/\s+/g, " ").trim(); if (!name || BAD_NAMES.has(normalizeName(name))) continue; fighters.push({ name, url: links[0].url }); } return fighters; }
async function buildUfcStatsIndex(fetchImpl: typeof fetch) { const letters = "abcdefghijklmnopqrstuvwxyz".split(""); const items: FighterIndexItem[] = []; for (const letter of letters) { try { items.push(...parseIndexPage(await fetchText(`http://www.ufcstats.com/statistics/fighters?char=${letter}&page=all`, fetchImpl))); } catch { } } const byName = new Map<string, FighterIndexItem>(); for (const item of items) byName.set(normalizeName(item.name), item); return byName; }
function findIndexItem(fighterName: string, index: Map<string, FighterIndexItem>): FighterMatch | null { const normalized = normalizeName(fighterName); const direct = index.get(normalized); if (direct) return { ...direct, matchMethod: "exact", matchScore: 1 }; for (const alias of NAME_ALIASES[normalized] ?? []) { const item = index.get(normalizeName(alias)); if (item) return { ...item, matchMethod: `alias:${alias}`, matchScore: 0.98 }; } const sorted = sortedNameKey(normalized); for (const item of index.values()) { if (sortedNameKey(item.name) === sorted) return { ...item, matchMethod: "token-sort", matchScore: 0.96 }; } const scored = [...index.values()].map((item) => ({ item, score: tokenScore(normalized, item.name) })).filter(({ score }) => score >= 0.86).sort((a, b) => b.score - a.score || a.item.name.length - b.item.name.length); const best = scored[0]; return best ? { ...best.item, matchMethod: "token-fuzzy", matchScore: Number(best.score.toFixed(3)) } : null; }
