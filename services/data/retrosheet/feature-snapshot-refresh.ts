...
export async function getRetrosheetFeatureSnapshotStatus(sourceKey = DEFAULT_SOURCE_KEY) {
  const [
    ...
    db.retrosheetGame.count({ where: { sourceKey } }),
    db.retrosheetTeamGameStat.count({ where: { sourceKey } }),
    db.retrosheetPitchingGameStat.count({ where: { sourceKey } }),
    db.mlbTeamEloSnapshot.count({ where: { sourceKey } }),
    db.mlbPitcherRollingSnapshot.count({ where: { sourceKey } })
  ]);
...
export async function refreshRetrosheetFeatureSnapshots(options: {
  ...
  const games = (await db.retrosheetGame.findMany({
    where: {
      sourceKey,
      homeScore: { not: null },
      awayScore: { not: null }
  ...
  const pitchingRows = (await db.retrosheetPitchingGameStat.findMany({
    where: { sourceKey },
    orderBy: [
      { gameDate: "asc" },
      { retrosheetGameId: "asc" }
  ...
  if (options.rebuild) {
    await db.mlbTeamEloSnapshot.deleteMany({ where: { sourceKey } });
    await db.mlbPitcherRollingSnapshot.deleteMany({ where: { sourceKey } });
  }
  ...
  await chunkCreate(pitcherSnapshots, async (chunk) => {
    await db.mlbPitcherRollingSnapshot.createMany({
      data: chunk,
      skipDuplicates: true
    });
  });
