...
export function normalizeDataballrPlayerContext(payload: unknown, request: DataballrPlayerContextRequest): DataballrPlayerContext | null {
  ...
  return {
    ...
    seasonAvg: pickNumber(raw, ["seasonAvg", "season_avg", "avg", "stat_avg", "points_per_game"]),
    last5Avg: pickNumber(raw, ["last5Avg", "last_5_avg", "last5", "l5_avg"]),
    last10Avg: pickNumber(raw, ["last10Avg", "last_10_avg", "last10", "l10_avg"]),
    seasonMinutes: pickNumber(raw, ["seasonMinutes", "season_minutes", "minutes", "mp"]),
    last5Minutes: pickNumber(raw, ["last5Minutes", "last_5_minutes", "l5_minutes"]),
    last10Minutes: pickNumber(raw, ["last10Minutes", "last_10_minutes", "l10_minutes"]),
    ...
    opponentPace: pickNumber(raw, ["opponentPace", "opponent_pace"]),
    opponentDefRating: pickNumber(raw, ["opponentDefRating", "opponent_def_rating", "opp_def_rating"]),
    opponentRankVsPosition: pickNumber(raw, ["opponentRankVsPosition", "opponent_rank_vs_position", "opp_rank_position"]),
    trueShootingPct: pickNumber(raw, ["trueShootingPct", "true_shooting_pct", "ts_pct"]),
    projectedMinutes: pickNumber(raw, ["projectedMinutes", "projected_minutes", "proj_minutes"]),
...
export async function fetchDataballrPlayerContext(request: DataballrPlayerContextRequest): Promise<DataballrPlayerContext | null> {
  ...
  const path = process.env.DATABALLR_PLAYER_CONTEXT_PATH?.trim() || "/nba/player-context";
