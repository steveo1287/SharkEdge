...
import {
  allSkillValues,
  buildUfcFighterSkillProfile,
  type UfcModelFeatureSnapshot
} from "@/services/ufc/fighter-skill-profile";
...
const strongFeature: UfcModelFeatureSnapshot = {
  fightId: "fight-1",
  fightDate,
  fighterId: "wrestler-a",
  opponentFighterId: "striker-b",
  snapshotAt,
  modelVersion: "v1",
  age: 29,
...
for (const value of allSkillValues(strong)) {
  assert.ok(value >= 0 && value <= 100, `skill ${value} should normalize to 0-100`);
}
...
assert.ok(Math.abs(lowSample.wrestling.takedownOffense - 50) < Math.abs(strong.wrestling.takedownOffense - 50), "low sample should shrink toward baseline");
...
assert.ok(recencyProfile.wrestling.takedownOffense < strong.wrestling.takedownOffense, "history should affect weighted skill profile");

assert.throws(() => buildUfcFighterSkillProfile({
  feature: { ...strongFeature, snapshotAt: "2026-06-02T00:00:00.000Z" }
}), /future-data leakage/);
