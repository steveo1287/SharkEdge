import { calculateEdgeScore, getEdgeBand } from "@/lib/utils/edge-score";
...
function buildLiveEdgeScore(game: LiveGame) {
  // Prefer the real sharkscore computed by the Python analytics engine.
  // It uses true EV, book consensus, vig efficiency, and line movement signals.
  const sharkscore = game.edge_analytics?.sharkscore;
  if (sharkscore && typeof sharkscore.score === "number") {
    const score = Math.round(Math.min(100, Math.max(0, sharkscore.score)));
    return { score, label: getEdgeBand(score) };
  }

  // Fallback: heuristic from available market data when backend analytics absent.
