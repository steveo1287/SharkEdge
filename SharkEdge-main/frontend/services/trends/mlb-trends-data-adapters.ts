...
async function loadHistoricalHarvestResponse() {
  const response = await fetch(`${getBackendBaseUrl()}/api/historical/odds/harvest?sport_key=baseball_mlb`, {
    cache: "no-store",
...
export async function loadRawMlbBoardTrendInputs(): Promise<unknown[]> {
  ...
  const response = selectPreferredBoardResponse([
    ...
    theRundownResponse ? { providerKey: therundownCurrentOddsProvider.key, response: theRundownResponse } : null
  ]);
