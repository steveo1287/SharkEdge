...
export async function getLivePropsExplorerData(filters: PropFilters) {
  ...
  // Collect backend-ranked top props — EV/Kelly sorted by the Python analytics engine.
  const backendTopProps = responses.flatMap((response) => response.top_props ?? []);

  return {
    filters,
    props: filteredProps,
    topProps: backendTopProps,
