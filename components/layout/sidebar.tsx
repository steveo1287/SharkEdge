...
import { MAIN_NAV_ITEMS, NAV_ICONS, isActivePath } from "./navigation";
...
function NavIcon({ svgContent }: { svgContent: string }) {
  return (
    <svg viewBox="0 0 16 16" className="h-[15px] w-[15px] shrink-0" aria-hidden="true" dangerouslySetInnerHTML={{ __html: svgContent }} />
  );
}

function NavLink({
  href, label, icon, badge, active, onNavigate
}: { href: string; label: string; icon: string; badge?: string; active: boolean; onNavigate?: () => void }) {
  return (
    <Link
      href={href}
      onClick={onNavigate}
      data-active={active ? "true" : undefined}
      className={cn("nav-item group", active && "text-text-primary")}
    >
      <span className={cn("shrink-0 transition-colors", active ? "text-aqua" : "text-bone/40 group-hover:text-bone/70")}>
  ...
  return (
    <div className={cn("flex h-full flex-col", mobile ? "w-[272px] bg-abyss" : "bg-abyss")}>
      ...
      <div className="flex items-center justify-between border-b border-bone/[0.06] px-5 py-[18px]">
        <BrandMark compact />
        <div className="flex items-center gap-1.5"><span className="live-dot" /></div>
      </div>
