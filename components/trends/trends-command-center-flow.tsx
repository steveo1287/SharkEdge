...
function has(card: TrendCardView, pattern: RegExp) {
  return pattern.test(words(card));
}
...
function verified(card: TrendCardView) {
  return has(card, /SAVED LEDGER VERIFIED|EVENTMARKET BACKTEST|LEDGER VERIFIED|\b\d+\s+GRADED\b/);
}

function needsProof(card: TrendCardView) {
  return has(card, /SEED STARTER|SEEDED|NO GRADED LEDGER|PRICE NEEDED|CURRENT PRICE REQUIRED|NO CURRENT QUALIFYING PRICE|OPEN ROWS|NO GRADE/);
}

function priced(card: TrendCardView) {
  return Boolean(card.priceCheckpoint) || has(card, /PRICE ATTACHED|MARKET-EDGE|REVIEW LIVE PRICE|FAIR \+|-?\d{3}/);
}

function current(card: TrendCardView) {
  return has(card, /CURRENT SIGNAL|SIM-ENGINE|MARKET-EDGE|REAL CURRENT/);
}

function prop(card: TrendCardView) {
  return /player_|prop|strikeout|rebounds|assists|points|threes|total bases|fight|round|method/i.test(`${card.market ?? ""} ${card.betType ?? ""} ${card.title ?? ""}`);
}

function mover(card: TrendCardView) {
  return !prop(card) && has(card, /STEAM|MOVE|MOVED|MOVEMENT|REPRICE|LINE|PRICE|BEST PRICE|MARKET-EDGE|CLV|COPY/);
}

function split(card: TrendCardView) {
  return has(card, /PUBLIC|BETS|HANDLE|MONEY %|TICKET|SPLIT|REVERSE LINE/);
}
...
function market(value: string | null | undefined) {
  return value ? value.replace(/_/g, " ") : "market";
}
...
function ToneText({ tone, children }: { tone: Tone; children: React.ReactNode }) {
  ...
  return <div className={`text-[10px] font-semibold uppercase tracking-[0.24em] ${cls}`}>{children}</div>;
}

function Pill({ children, kind = "muted" }: { children: React.ReactNode; kind?: "good" | "warn" | "info" | "muted" }) {
  ...
  return <span className={`rounded-full border px-2 py-0.5 text-[10px] font-semibold uppercase tracking-[0.13em] ${cls}`}>{children}</span>;
}

function Gate({ value }: { value?: string }) {
  const gate = value ?? "RESEARCH ONLY";
  const kind = gate.toUpperCase().includes("REVIEW") ? "good" : gate.toUpperCase().includes("WATCH") ? "info" : gate.toUpperCase().includes("CONTEXT") ? "warn" : "muted";
  return <Pill kind={kind}>{gate}</Pill>;
}
