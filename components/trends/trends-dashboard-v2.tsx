...
import type { TrendCardView, TrendDashboardView, TrendFilters, TrendMode } from "@/lib/types/domain";
import { TREND_QUERY_EXAMPLES } from "@/services/trends/ai-query";

type TrendsDashboardV2Props = {
...
function extractActionGate(card: TrendCardView) {
  const sources = [card.note, card.whyItMatters, card.caution, ...(card.todayMatches ?? []).map((match) => match.recommendedBetLabel ?? "")];
...
function extractPriceNeeded(card: TrendCardView) {
  const text = [card.note, card.whyItMatters, card.caution, ...(card.todayMatches ?? []).map((match) => match.oddsContext ?? "")]
    .filter(Boolean)
    .join(" | ");
  const explicit = text.match(/Fair-price checkpoint:\s*([^|.]+)/i)?.[1]?.trim();
  if (explicit) return explicit;
  return card.roi ? `ROI checkpoint: ${card.roi}. Do not chase a worse number.` : "Price checkpoint unavailable. Verify current board price before using this trend.";
}

function extractKillSwitches(card: TrendCardView) {
  const explicit = card.caution?.replace(/^Kill switches:\s*/i, "").trim();
  if (explicit) return explicit;
  if ((card.todayMatches ?? []).length === 0) return "No current qualifier. Treat as historical context until a game matches.";
...
function getQualityRead(card: TrendCardView): QualityRead {
  ...
  return {
    ...
    whySurfaced: compactText(card.whyItMatters || card.explanation || card.note, "The system surfaced this card from stored trend and market context."),
    liveAgreement: liveCount
      ? `${liveCount} live qualifier${liveCount === 1 ? "" : "s"} attached. Check board/props links for live price agreement.`
      : "No live qualifier attached. Use as research/watchlist context."
  };
}
...
export function TrendsDashboardV2({ data }: TrendsDashboardV2Props) {
  if (data.setup) {
  ...
  return (
    <div className="grid gap-6">
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <DataTable title="Movement / action gates" rows={data.movementRows} />
        <DataTable title="Trend segments" rows={data.segmentRows} />
      </div>
    </div>
  );
}
