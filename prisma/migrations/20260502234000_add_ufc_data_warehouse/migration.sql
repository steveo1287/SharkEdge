...
-- All model/backtest features must be snapshotted at or before the fight date.
...
CREATE TABLE IF NOT EXISTS ufc_model_features (
  ...
  created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
  updated_at TIMESTAMPTZ NOT NULL DEFAULT now(),
  CONSTRAINT ufc_model_features_snapshot_no_future_chk CHECK (snapshot_at <= (SELECT fight_date FROM ufc_fights WHERE ufc_fights.id = fight_id))
);
